...
# price_precision, qty_precision = self.exchange.get_symbol_precision_bybit(symbol)
    ...
    def calculate_dynamic_outer_price_distance(self, spread, min_outer_price_distance, max_outer_price_distance_long, max_outer_price_distance_short):
        ...
        logging.info(f"(caller: {inspect.stack()[1].function}, func: {inspect.currentframe().f_code.co_name}, line: {inspect.currentframe().f_lineno}) [symbol] Dynamic outer price distance (Long): {dynamic_outer_price_distance_long}")
        logging.info(f"(caller: {inspect.stack()[1].function}, func: {inspect.currentframe().f_code.co_name}, line: {inspect.currentframe().f_lineno}) [symbol] Dynamic outer price distance (Short): {dynamic_outer_price_distance_short}")
    ...
    def calculate_buffer_distances(self, current_price, buffer_percentage_long, buffer_percentage_short):
        buffer_distance_long = current_price * buffer_percentage_long
        buffer_distance_short = current_price * buffer_percentage_short
        logging.info(f"(caller: {inspect.stack()[1].function}, func: {inspect.currentframe().f_code.co_name}, line: {inspect.currentframe().f_lineno}) Long buffer distance: {buffer_distance_long}, Short buffer distance: {buffer_distance_short}")
        return buffer_distance_long, buffer_distance_short
    ...
    def finalize_grid_levels(self, adjusted_grid_levels_long, adjusted_grid_levels_short, levels, current_price, buffer_distance_long, buffer_distance_short, max_outer_price_distance_long, max_outer_price_distance_short, initial_entry_long, initial_entry_short):
        ...
        logging.info(f"(caller: {inspect.stack()[1].function}, func: {inspect.currentframe().f_code.co_name}, line: {inspect.currentframe().f_lineno}) [symbol] Long grid levels: {adjusted_grid_levels_long}")
        logging.info(f"(caller: {inspect.stack()[1].function}, func: {inspect.currentframe().f_code.co_name}, line: {inspect.currentframe().f_lineno}) [symbol] Short grid levels: {adjusted_grid_levels_short}")
    ...
    def issue_grid_orders(self, symbol: str, side: str, grid_levels: list, amounts: list, is_long: bool, filled_levels: set):
        ...
        try:
            ...
            for level, amount in zip(grid_levels, amounts):
                ...
                logging.debug(f"(caller: {inspect.stack()[1].function}, func: {inspect.currentframe().f_code.co_name}, line: {inspect.currentframe().f_lineno}) [[{symbol}]] Attempting to place {side} order for [{symbol}]: Level={level}, Amount={amount}")

                # **Check if the amount is 0.0 before proceeding**
                if amount == 0.0:
                    logging.info(f"(caller: {inspect.stack()[1].function}, func: {inspect.currentframe().f_code.co_name}, line: {inspect.currentframe().f_lineno}) [[{symbol}]] Skipping {side} order at level {level} for [{symbol}] due to 0.0 amount.")
                ...
                if not order_exists:
                    ...
                    try:
                        ...
                        logging.info(f"(caller: {inspect.stack()[1].function}, func: {inspect.currentframe().f_code.co_name}, line: {inspect.currentframe().f_lineno}) [[{symbol}]] Placing {side} limit order for [{symbol}] at level {level} with amount {amount}. OrderLinkId: {order_link_id}, PositionIdx: {position_idx}")
