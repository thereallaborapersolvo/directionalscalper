...
class BybitStrategy(BaseStrategy):
    def __init__(self, exchange, config, manager, symbols_allowed=None):
        ...
        logging.info(f"Function __init__ called")
        ...
        # Initialize the dictionary to manage symbol-specific locks
        self.symbol_locks = {}
        
        # **Initialize the grid_lock**
        self.grid_lock = Lock()

 
    def start_hotkey_listener(self):

        logging.info("Function start_hotkey_listener called")
        hotkey_thread = threading.Thread(target=self.listen_hotkeys, daemon=True)
        hotkey_thread.start()

    def listen_hotkeys(self):

        logging.info("Function listen_hotkeys called")
        while True:
    ...
    def hotkey_trading_strategy(self, open_orders: list, symbol: str, total_equity: float, long_pos_qty: float, short_pos_qty: float, long_pos_price: float, short_pos_price: float, long_dynamic_amount: float, short_dynamic_amount: float, upnl_profit_pct: float, tp_order_counts: dict):
        ...
        logging.info(f"[[{symbol}]] Function hotkey_trading_strategy called")
        try:
    ...
    def generate_l_signals(self, symbol):
        ...
        logging.info(f"[[{symbol}]] Function generate_l_signals called")
    ...
    def get_market_data_with_retry(self, symbol, max_retries=5, retry_delay=5):
        ...
        logging.info(f"[[{symbol}]] Function get_market_data_with_retry called")
    ...
    def update_dynamic_amounts(self, symbol, total_equity, best_ask_price, best_bid_price):
        ...
        logging.info(f"[[{symbol}]] Function update_dynamic_amounts called")
    ...
    def get_open_symbols(self):
        ...
        logging.info("Function get_open_symbols called")
    ...
    def place_reduce_only_limit_order(exchange, symbol, side, quantity, stop_loss_price, positionIdx=0):
        ...
        logging.info(f"[[{symbol}]] Function place_reduce_only_limit_order called")
        """
    ...
    def execute_grid_auto_reduce(self, position_type, symbol, pos_qty, dynamic_amount, market_price, total_equity, long_pos_price, short_pos_price, min_qty):
        ...
        logging.info(f"[[{symbol}]] Function execute_grid_auto_reduce called")
        """
    ...
    def calculate_step_price(self, position_type, market_price, price_interval, level):
        ...
        logging.info("Function calculate_step_price called")
        """
    ...
    def is_price_valid(self, position_type, step_price, market_price):
        ...
        logging.info("Function is_price_valid called")
        """
    ...
    def place_tagged_limit_order(self, symbol, side, amount, price, reduce_only, tag):
        ...
        logging.info("Function place_tagged_limit_order called")
        """
        Places a tagged limit order with the option for it to be a reduce-only order.
        """
        try:
            logging.info(f"[[{symbol}]] Placing order: {side.capitalize()} at level {price:.4f} with tag {tag} and amount {amount:.4f}")
    ...
    def check_symbol_inactivity(self, symbol, inactive_time_threshold):
        ...
        logging.info(f"[[{symbol}]] Function check_symbol_inactivity called")
    ...
    def handle_inactivity(self, symbol, side, current_time, inactive_time_threshold, previous_qty):
        ...
        logging.info(f"[[{symbol}]] Function handle_inactivity called")
    ...
    def check_position_inactivity(self, symbol, inactive_pos_time_threshold, long_pos_qty, short_pos_qty, previous_long_pos_qty, previous_short_pos_qty):
        ...
        logging.info(f"[[{symbol}]] Function check_position_inactivity called")
        current_time = time.time()
        logging.info(f"[[{symbol}]] Checking position inactivity for symbol at {current_time}")

        has_open_long_position = long_pos_qty > 0
        has_open_short_position = short_pos_qty > 0

        logging.info(f"[[{symbol}]] Open positions status for symbol - Long: {'found' if has_open_long_position else 'none'}, Short: {'found' if has_open_short_position else 'none'}")
    ...
    def should_terminate_open_orders(self, symbol, long_pos_qty, short_pos_qty, open_positions_data, open_orders, current_time):
        ...
        logging.info(f"[[{symbol}]] Function should_terminate_open_orders called")
        try:
    ...
    def should_terminate_full(self, symbol, current_time, previous_long_pos_qty, long_pos_qty, previous_short_pos_qty, short_pos_qty):
        ...
        logging.info(f"[[{symbol}]] Function should_terminate_full called")
    ...
    def should_terminate(self, symbol, current_time):
        ...
        logging.info(f"[[{symbol}]] Function should_terminate called")
        open_symbols = self.get_open_symbols()  # Fetch open symbols
        if symbol not in open_symbols:
            if not hasattr(self, 'position_closed_time'):
                self.position_closed_time = current_time
    ...
    def cleanup_before_termination(self, symbol):
        ...
        logging.info(f"[[{symbol}]] Function cleanup_before_termination called")
    ...
    def calculate_next_update_time(self):
        ...
        logging.info("Function calculate_next_update_time called")
    ...
    def cancel_entries_bybit(self, symbol, best_ask_price, ma_1m_3_high, ma_5m_3_high):
        ...
        logging.info(f"[[{symbol}]] Function cancel_entries_bybit called")
        # Cancel entries
        current_time = time.time()
    ...
    def clear_stale_positions(self, open_orders, rotator_symbols, max_time_without_volume=3600): # default time is 1 hour
        ...
        logging.info(f"[[{symbol}]] Function clear_stale_positions called")
    ...
    def cancel_stale_orders_bybit(self, symbol):
        ...
        logging.info(f"[[{symbol}]] Function cancel_stale_orders_bybit called")
        current_time = time.time()
    ...
    def cancel_all_orders_for_symbol_bybit(self, symbol):
        ...
        logging.info(f"[[{symbol}]] Function cancel_all_orders_for_symbol_bybit called")
        try:
            self.exchange.cancel_all_open_orders_bybit(symbol)
    ...
    def get_all_open_orders_bybit(self):
        ...
        logging.info("Function get_all_open_orders_bybit called")
        """
    ...
    def cancel_old_entries_bybit(self, symbol):
        ...
        logging.info(f"[[{symbol}]] Function cancel_old_entries_bybit called")
        # Cancel entries
        try:
            self.exchange.cancel_all_entries_bybit(symbol)
            logging.info(f"Canceled entry orders for [[{symbol}]]")
    ...
    def update_quickscalp_tp_dynamic(self, symbol, pos_qty, upnl_profit_pct, max_upnl_profit_pct, short_pos_price, long_pos_price, positionIdx, order_side, last_tp_update, tp_order_counts, open_orders):
        ...
        logging.info(f"[[{symbol}]] Function update_quickscalp_tp_dynamic called")
        ...
        for order in mismatched_qty_orders:
            try:
                ...
                logging.info(f"[[{symbol}]] Cancelled TP order {order['id']} for update.")
            except Exception as e:
                logging.info(f"[[{symbol}]] Error in cancelling {order_side} TP order {order['id']}. Error: {e}")

        # Debug log to inspect the current time and last TP update time
        now = datetime.now()
        logging.info(f"[[{symbol}]] Inside update_quickscalp_tp_dynamic - Current time: {now}, TP update time before adjustment: {last_tp_update}")
        ...
        if now >= last_tp_update or mismatched_qty_orders:
            ...
            if not tp_order_exists:
                ...
                logging.info(f"[[{symbol}]] New tp price min: {new_tp_price_min} with order side as {order_side}")
                ...
                if (order_side == "sell" and current_price >= new_tp_price_min) or (order_side == "buy" and current_price <= new_tp_price_max):
                    ...
                    if (order_side == "sell" and current_price > new_tp_price_max) or (order_side == "buy" and current_price < new_tp_price_min):
                        try:
                            ...
                            logging.info(f"[[{symbol}]] New {order_side.capitalize()} TP set at current/best price {tp_price} as current price has surpassed the max TP")
                        except Exception as e:
                            logging.info(f"[[{symbol}]] Failed to set new {order_side} TP for [[{symbol}]] at current/best price. Error: {e}")
                    else:
                        try:
                            ...
                            logging.info(f"[[{symbol}]] New {order_side.capitalize()} TP set at {new_tp_price_min} using a normal limit order")
                        except Exception as e:
                            logging.info(f"[[{symbol}]] Failed to set new {order_side} TP for [[{symbol}]] using a normal limit order. Error: {e}")
                else:
                    try:
                        ...
                        logging.info(f"[[{symbol}]] New {order_side.capitalize()} TP set at {new_tp_price_max} using a post-only order")
                    except Exception as e:
                        logging.info(f"[[{symbol}]] Failed to set new {order_side} TP for [[{symbol}]] using a post-only order. Error: {e}")
            else:
                logging.info(f"[[{symbol}]] Skipping TP update as a TP order already exists for [[{symbol}]]")
            ...
            if order_side == "sell":
                ...
                logging.info(f"[[{symbol}]] Updated long TP time: {self.next_long_tp_update}")
            else:
                self.next_short_tp_update = now + timedelta(seconds=15)  # Adjust time as per strategy needs
                logging.info(f"[[{symbol}]] Updated short TP time: {self.next_short_tp_update}")

            # Return the appropriate updated time
            return self.next_long_tp_update if order_side == "sell" else self.next_short_tp_update
        else:
            logging.info(f"[[{symbol}]] No immediate update needed for TP orders for [[{symbol}]]. Last update at: {last_tp_update}")
    ...
    def update_quickscalp_tp(self, symbol, pos_qty, upnl_profit_pct, short_pos_price, long_pos_price, positionIdx, order_side, last_tp_update, tp_order_counts, open_orders, max_retries=10):
        ...
        logging.info(f"[[{symbol}]] Function update_quickscalp_tp called")
        # Fetch the current open TP orders and TP order counts for the symbol
            ...
            if not tp_order_exists:
                ...
                if (order_side == "sell" and current_price >= new_tp_price) or (order_side == "buy" and current_price <= new_tp_price):
                    ...
                    try:
                        ...
                        logging.info(f"[[{symbol}]] New {order_side.capitalize()} TP set at {new_tp_price} using a normal limit order")
                    except Exception as e:
                        logging.info(f"[[{symbol}]] Failed to set new {order_side} TP for [[{symbol}]] using a normal limit order. Error: {e}")
                else:
                    ...
                    try:
                        ...
                        logging.info(f"[[{symbol}]] New {order_side.capitalize()} TP set at {new_tp_price} using a post-only order")
                    except Exception as e:
                        logging.info(f"[[{symbol}]] Failed to set new {order_side} TP for [[{symbol}]] using a post-only order. Error: {e}")
            else:
                logging.info(f"[[{symbol}]] Skipping TP update as a TP order already exists for [[{symbol}]]")
    ...
    def calculate_quickscalp_long_take_profit_dynamic_distance(self, long_pos_price, symbol, min_upnl_profit_pct, max_upnl_profit_pct):
        ...
        logging.info(f"[[{symbol}]] Function calculate_quickscalp_long_take_profit_dynamic_distance called")
        if long_pos_price is None or long_pos_price <= 0:
            logging.info(f"[[{symbol}]] Invalid long position price for {symbol}: {long_pos_price}")
            return None, None

        price_precision = int(self.exchange.get_price_precision(symbol))
        logging.info(f"[[{symbol}]] Price precision for [[{symbol}]]: {price_precision}")
        ...
        logging.info(f"[[{symbol}]] Calculated raw TP values for {symbol} - Min TP: {min_target_profit_price}, Max TP: {max_target_profit_price}")
        ...
        try:
            ...
            logging.info(f"[[{symbol}]] Error when quantizing target_profit_prices for {symbol}. {e}")
            return None, None

        # Log the quantized TP values before returning
        logging.info(f"[[{symbol}]] Quantized TP values for {symbol} - Min TP: {float(min_target_profit_price)}, Max TP: {float(max_target_profit_price)}")
    ...
    def calculate_quickscalp_short_take_profit_dynamic_distance(self, short_pos_price, symbol, min_upnl_profit_pct, max_upnl_profit_pct):
        ...
        logging.info(f"[[{symbol}]] Function calculate_quickscalp_short_take_profit_dynamic_distance called")
        if short_pos_price is None or short_pos_price <= 0:
            logging.info(f"[[{symbol}]] Invalid short position price for {symbol}: {short_pos_price}")
            return None, None
    ...
    def calculate_quickscalp_long_take_profit(self, long_pos_price, symbol, upnl_profit_pct):
        ...
        logging.info(f"[[{symbol}]] Function calculate_quickscalp_long_take_profit called")
    ...
    def calculate_quickscalp_short_take_profit(self, short_pos_price, symbol, upnl_profit_pct):
        ...
        logging.info(f"[[{symbol}]] Function calculate_quickscalp_short_take_profit called")
...
# price_precision, qty_precision = self.exchange.get_symbol_precision_bybit(symbol)
    def calculate_dynamic_long_take_profit(self, best_bid_price, long_pos_price, symbol, upnl_profit_pct, max_deviation_pct=0.0040):
        ...
        logging.info(f"[[{symbol}]] Function calculate_dynamic_long_take_profit called")
        if long_pos_price is None:
    ...
    def calculate_dynamic_short_take_profit(self, best_ask_price, short_pos_price, symbol, upnl_profit_pct, max_deviation_pct=0.05):
        ...
        logging.info(f"[[{symbol}]] Function calculate_dynamic_short_take_profit called")
        if short_pos_price is None:
    ...
    def detect_order_book_walls(self, symbol, threshold=5.0):
        ...
        logging.info(f"[[{symbol}]] Function detect_order_book_walls called")
        order_book = self.exchange.get_orderbook(symbol)
    ...
    def detect_significant_order_book_walls_atr(self, symbol, timeframe='1h', base_threshold_factor=5.0, atr_proximity_percentage=10.0):
        ...
        logging.info(f"[[{symbol}]] Function detect_significant_order_book_walls_atr called")
        order_book = self.exchange.get_orderbook(symbol)
        ...
        def is_wall_significant(price, size, threshold, avg_size):
            ...
            logging.info(f"[[{symbol}]] Function is_wall_significant called")
    ...
    def is_price_approaching_wall(self, current_price, wall_price, wall_type):
        ...
        logging.info("Function is_price_approaching_wall called")
    ...
    def calculate_trading_fee(self, qty, executed_price, fee_rate=TAKER_FEE_RATE):
        ...
        logging.info("Function calculate_trading_fee called")
    ...
    def calculate_orderbook_strength(self, symbol, depth=10):
        ...
        logging.info(f"[[{symbol}]] Function calculate_orderbook_strength called")
        order_book = self.exchange.get_orderbook(symbol)
    ...
    def initialize_symbol(self, symbol, total_equity, best_ask_price, max_leverage):
        ...
        logging.info(f"[[{symbol}]] Function initialize_symbol called")
    ...
    def adjust_risk_parameters_qstrend(self, exchange_max_leverage):
        ...
        logging.info("Function adjust_risk_parameters_qstrend called")
        """
    ...
    def adjust_risk_parameters(self, exchange_max_leverage):
        ...
        logging.info("Function adjust_risk_parameters called")
        """
        Adjust risk parameters based on user preferences.
    ...
    def handle_trade_quantities(self, symbol, total_equity, best_ask_price):
        ...
        logging.info(f"[[{symbol}]] Function handle_trade_quantities called")
        if symbol not in self.initialized_symbols:
    ...
    def calculate_max_trade_qty(self, symbol, total_equity, best_ask_price, max_leverage):
        ...
        logging.info(f"[[{symbol}]] Function calculate_max_trade_qty called")
    ...
    def check_amount_validity_bybit(self, amount, symbol):
        ...
        logging.info(f"[[{symbol}]] Function check_amount_validity_bybit called")
    ...
    def check_amount_validity_once_bybit(self, amount, symbol):
        ...
        logging.info(f"[[{symbol}]] Function check_amount_validity_once_bybit called")
    ...
    def can_proceed_with_trade_funding(self, symbol: str) -> dict:
        ...
        logging.info(f"[[{symbol}]] Function can_proceed_with_trade_funding called")
        """
    ...
    def can_place_order(self, symbol, interval=60):
        ...
        logging.info(f"[[{symbol}]] Function can_place_order called")
        with self.lock:
            current_time = time.time()
    ...
    def bybit_hedge_placetp_maker(self, symbol, pos_qty, take_profit_price, positionIdx, order_side, open_orders):
        ...
        logging.info(f"[[{symbol}]] Function bybit_hedge_placetp_maker called")
    ...
    def postonly_limit_order_bybit(self, symbol, side, amount, price, positionIdx, reduceOnly=False):
        ...
        logging.info(f"[[{symbol}]] Function postonly_limit_order_bybit called")
    ...
    def place_hedge_order_bybit(self, symbol, side, amount, price, positionIdx, reduceOnly=False):
        ...
        logging.info(f"[[{symbol}]] Function place_hedge_order_bybit called")
    ...
    def place_postonly_order_bybit(self, symbol, side, amount, price, positionIdx, reduceOnly=False):
        ...
        logging.info(f"[[{symbol}]] Function place_postonly_order_bybit called")
    ...
    def postonly_limit_entry_order_bybit(self, symbol, side, amount, price, positionIdx, reduceOnly=False):
        ...
        logging.info(f"[[{symbol}]] Function postonly_limit_entry_order_bybit called")
    ...
    def create_normal_stop_loss_order_bybit(self, symbol, order_type, side, amount, price=None, positionIdx=None, reduce_only=True):
        ...
        logging.info(f"[[{symbol}]] Function create_normal_stop_loss_order_bybit called")
        """
    ...
    def limit_order_bybit_reduce_nolimit(self, symbol, side, amount, price, positionIdx, reduceOnly=False):
        ...
        logging.info(f"[[{symbol}]] Function limit_order_bybit_reduce_nolimit called")
    ...
    def postonly_limit_order_bybit_nolimit(self, symbol, side, amount, price, positionIdx, reduceOnly=False):
        ...
        logging.info(f"[[{symbol}]] Function postonly_limit_order_bybit_nolimit called")
        params = {"reduceOnly": reduceOnly, "postOnly": True}
        logging.info(f"Placing {side} limit order for [[{symbol}]] at {price} with qty {amount} and params {params}...")
        try:
            order = self.exchange.create_limit_order_bybit(symbol, side, amount, price, positionIdx=positionIdx, params=params)
    ...
    def limit_order_bybit_nolimit(self, symbol, side, amount, price, positionIdx, reduceOnly=False):
        ...
        logging.info(f"[[{symbol}]] Function limit_order_bybit_nolimit called")
    ...
    def postonly_limit_order_bybit_s(self, symbol, side, amount, price, positionIdx, reduceOnly=False):
        ...
        logging.info(f"[[{symbol}]] Function postonly_limit_order_bybit_s called")
        params = {"reduceOnly": reduceOnly, "postOnly": True}
        logging.info(f"Placing {side} limit order for [[{symbol}]] at {price} with qty {amount} and params {params}...")
        try:
            order = self.exchange.create_limit_order_bybit(symbol, side, amount, price, positionIdx=positionIdx, params=params)
    ...
    def limit_order_bybit(self, symbol, side, amount, price, positionIdx, reduceOnly=False):
        ...
        logging.info(f"[[{symbol}]] Function limit_order_bybit called")
        params = {"reduceOnly": reduceOnly}
    ...
    def limit_order_bybit_unified(self, symbol, side, amount, price, positionIdx, reduceOnly=False):
        ...
        logging.info(f"[[{symbol}]] Function limit_order_bybit_unified called")
        params = {"reduceOnly": reduceOnly}
        #print(f"Symbol: [[{symbol}]], Side: {side}, Amount: {amount}, Price: {price}, Params: {params}")
    ...
    def update_take_profit_if_profitable_for_one_minute(self, symbol):
        ...
        logging.info(f"[[{symbol}]] Function update_take_profit_if_profitable_for_one_minute called")
        try:
    ...
    def update_dynamic_quickscalp_tp(self, symbol, best_ask_price, best_bid_price, pos_qty, upnl_profit_pct, short_pos_price, long_pos_price, positionIdx, order_side, last_tp_update, tp_order_counts, max_retries=10):
        ...
        logging.info(f"[[{symbol}]] Function update_dynamic_quickscalp_tp called")
        # Fetch the current open TP orders and TP order counts for the symbol
        ...
        if now >= last_tp_update or mismatched_qty_orders:
            ...
            if not tp_order_exists:
                ...
                try:
                    self.exchange.create_take_profit_order_bybit(symbol, "limit", order_side, pos_qty, new_tp_price, positionIdx=positionIdx, reduce_only=True)
                    logging.info(f"[[{symbol}]] New {order_side.capitalize()} TP set at {new_tp_price}")
                except Exception as e:
                    logging.info(f"[[{symbol}]] Failed to set new {order_side} TP for [[{symbol}]]. Error: {e}")
            else:
                logging.info(f"[[{symbol}]] Skipping TP update as a TP order already exists for [[{symbol}]]")
    ...
    def place_long_tp_order(self, symbol, best_ask_price, long_pos_price, long_pos_qty, long_take_profit, open_orders):
        ...
        logging.info(f"[[{symbol}]] Function place_long_tp_order called")
        try:
    ...
    def place_short_tp_order(self, symbol, best_bid_price, short_pos_price, short_pos_qty, short_take_profit, open_orders):
        ...
        logging.info(f"[[{symbol}]] Function place_short_tp_order called")
        try:
    ...
    def entry_order_exists(self, open_orders, side):
        ...
        logging.info("Function entry_order_exists called")
    ...
    def calculate_dynamic_amounts_notional_nowelimit(self, symbol, total_equity, best_ask_price, best_bid_price, max_retries=20):
        ...
        logging.info(f"[[{symbol}]] Function calculate_dynamic_amounts_notional_nowelimit called")
        ...
        logging.info(f"[[{symbol}]] Market data for symbol: {market_data}")
        ...
        try:
            ...
            logging.info(f"[[{symbol}]] Calculated long entry size for symbol: {long_entry_size} units")
            logging.info(f"[[{symbol}]] Calculated short entry size for symbol: {short_entry_size} units")
    ...
    def calculate_dynamic_amounts_notional(self, symbol, total_equity, best_ask_price, best_bid_price, wallet_exposure_limit_long, wallet_exposure_limit_short, max_retries=20):
        ...
        logging.info(f"[[{symbol}]] Function calculate_dynamic_amounts_notional called")
        market_data = self.get_market_data_with_retry(symbol, max_retries=max_retries, retry_delay=5)
    ...
    def calculate_dynamic_amounts(self, symbol, total_equity, best_ask_price, best_bid_price):
        ...
        logging.info(f"[[{symbol}]] Function calculate_dynamic_amounts called")
        """
        ...
        logging.info(f"[[{symbol}]] Max long pos value for symbol : {max_long_position_value}")
        ...
        logging.info(f"[[{symbol}]] Max short pos value for symbol : {max_short_position_value}")
        ...
        logging.info(f"[[{symbol}]] Calculated long entry size for symbol: {long_entry_size_adjusted} units")
        logging.info(f"[[{symbol}]] Calculated short entry size for symbol: {short_entry_size_adjusted} units")
    ...
    def calculate_dynamic_amounts_notional_bybit_2(self, symbol, total_equity, best_ask_price, best_bid_price):
        ...
        logging.info(f"[[{symbol}]] Function calculate_dynamic_amounts_notional_bybit_2 called")
        """
        Calculate the dynamic entry sizes for both long and short positions based on wallet exposure limit and user-defined leverage,
    ...
    def calculate_dynamic_amounts_notional_bybit(self, symbol, total_equity, best_ask_price, best_bid_price):
        ...
        logging.info(f"[[{symbol}]] Function calculate_dynamic_amounts_notional_bybit called")
        """
        Calculate the dynamic entry sizes for both long and short positions based on wallet exposure limit and user-defined leverage,
        ensuring compliance with the exchange's minimum notional value requirements.
    ...
    def calculate_dynamic_amount_obstrength(self, symbol, total_equity, best_ask_price, max_leverage):
        ...
        logging.info(f"[[{symbol}]] Function calculate_dynamic_amount_obstrength called")
        self.initialize_trade_quantities(symbol, total_equity, best_ask_price, max_leverage)

        market_data = self.get_market_data_with_retry(symbol, max_retries=100, retry_delay=5)
        min_qty = float(market_data["min_qty"])
        logging.info(f"[[{symbol}]] Min qty for symbol : {min_qty}")
    ...
    def bybit_1m_mfi_quickscalp_trend_noeri_maxposbal(self, open_orders: list, symbol: str, min_vol: float, one_minute_volume: float, mfirsi: str, long_dynamic_amount: float, short_dynamic_amount: float, long_pos_qty: float, short_pos_qty: float, long_pos_price: float, short_pos_price: float, entry_during_autoreduce: bool, volume_check: bool, long_take_profit: float, short_take_profit: float, upnl_profit_pct: float, tp_order_counts: dict, total_equity: float, max_pos_balance_pct: float):
        ...
        logging.info(f"[[{symbol}]] Function bybit_1m_mfi_quickscalp_trend_noeri_maxposbal called")
        try:
            if symbol not in self.symbol_locks:
                self.symbol_locks[symbol] = threading.Lock()
    ...
    def bybit_1m_mfi_quickscalp_trend_noeri(self, open_orders: list, symbol: str, min_vol: float, one_minute_volume: float, mfirsi: str, long_dynamic_amount: float, short_dynamic_amount: float, long_pos_qty: float, short_pos_qty: float, long_pos_price: float, short_pos_price: float, entry_during_autoreduce: bool, volume_check: bool, long_take_profit: float, short_take_profit: float, upnl_profit_pct: float, tp_order_counts: dict):
        ...
        logging.info(f"[[{symbol}]] Function bybit_1m_mfi_quickscalp_trend_noeri called")
        try:
            if symbol not in self.symbol_locks:
                self.symbol_locks[symbol] = threading.Lock()
    ...
    def bybit_1m_mfi_quickscalp_ema_trend(self, open_orders: list, symbol: str, min_vol: float, one_minute_volume: float, mfirsi: str, ema_trend: str, long_dynamic_amount: float, short_dynamic_amount: float, long_pos_qty: float, short_pos_qty: float, long_pos_price: float, short_pos_price: float, entry_during_autoreduce: bool, volume_check: bool, long_take_profit: float, short_take_profit: float, upnl_profit_pct: float, tp_order_counts: dict):
        ...
        logging.info(f"[[{symbol}]] Function bybit_1m_mfi_quickscalp_ema_trend called")
        try:
            if symbol not in self.symbol_locks:
                self.symbol_locks[symbol] = threading.Lock()
    ...
    def bybit_1m_mfi_quickscalp_trend_eri(self, open_orders: list, symbol: str, min_vol: float, one_minute_volume: float, mfirsi: str, eri_trend: str, long_dynamic_amount: float, short_dynamic_amount: float, long_pos_qty: float, short_pos_qty: float, long_pos_price: float, short_pos_price: float, entry_during_autoreduce: bool, volume_check: bool, long_take_profit: float, short_take_profit: float, upnl_profit_pct: float, tp_order_counts: dict):
        ...
        logging.info(f"[[{symbol}]] Function bybit_1m_mfi_quickscalp_trend_eri called")
        try:
            if symbol not in self.symbol_locks:
                self.symbol_locks[symbol] = threading.Lock()
    ...
    def bybit_1m_mfi_quickscalp_trend_long_only(self, open_orders: list, symbol: str, min_vol: float, one_minute_volume: float, mfirsi: str, long_dynamic_amount: float, long_pos_qty: float, long_pos_price: float, volume_check: bool, long_take_profit: float, upnl_profit_pct: float, tp_order_counts: dict):
        ...
        logging.info(f"[[{symbol}]] Function bybit_1m_mfi_quickscalp_trend_long_only called")
        try:
            if symbol not in self.symbol_locks:
                self.symbol_locks[symbol] = threading.Lock()
    ...
    def bybit_1m_mfi_quickscalp_trend_short_only(self, open_orders: list, symbol: str, min_vol: float, one_minute_volume: float, mfirsi: str, short_dynamic_amount: float, short_pos_qty: float, short_pos_price: float, volume_check: bool, short_take_profit: float, upnl_profit_pct: float, tp_order_counts: dict):
        ...
        logging.info(f"[[{symbol}]] Function bybit_1m_mfi_quickscalp_trend_short_only called")
        try:
            if symbol not in self.symbol_locks:
                self.symbol_locks[symbol] = threading.Lock()
    ...
    def bybit_hedge_entry_maker_mfirsitrenderi(self, symbol, data, min_vol, min_dist, one_minute_volume, five_minute_distance, 
        ...
        logging.info(f"[[{symbol}]] Function bybit_hedge_entry_maker_mfirsitrenderi called")
    ...
    def bybit_hedge_entry_maker_mfirsitrend(self, symbol, data, min_vol, min_dist, one_minute_volume, five_minute_distance, 
        ...
        logging.info(f"[[{symbol}]] Function bybit_hedge_entry_maker_mfirsitrend called")
    ...
    def bybit_hedge_entry_maker_mfirsi(self, symbol, data, min_vol, min_dist, one_minute_volume, five_minute_distance, 
        ...
        logging.info(f"[[{symbol}]] Function bybit_hedge_entry_maker_mfirsi called")
    ...
    def bybit_1m_mfi_quickscalp_trend_long_only_spot(self, open_orders: list, symbol: str, min_vol: float, one_minute_volume: float, mfirsi: str, long_dynamic_amount: float, spot_position_qty: float, spot_position_price: float, volume_check: bool, upnl_profit_pct: float):
        ...
        logging.info(f"[[{symbol}]] Function bybit_1m_mfi_quickscalp_trend_long_only_spot called")
        try:
            if symbol not in self.symbol_locks:
                self.symbol_locks[symbol] = threading.Lock()
    ...
    def calculate_dynamic_outer_price_distance_atr(self, atrp, min_outer_price_distance, max_outer_price_distance):
        ...
        logging.info("Function calculate_dynamic_outer_price_distance_atr called")
        """
    ...
    def calculate_dynamic_outer_price_distance_normal(self, min_outer_price_distance: float, max_outer_price_distance: float) -> float:
        ...
        logging.info("Function calculate_dynamic_outer_price_distance_normal called")
        """
    ...
    def calculate_dynamic_outer_price_distance(self, order_book, current_price, max_outer_price_distance):
        ...
        logging.info("Function calculate_dynamic_outer_price_distance called")
    ...
    def calculate_dynamic_outer_price_distance_orderbook(self, order_book, current_price, max_outer_price_distance, min_outer_price_distance):
        ...
        logging.info("Function calculate_dynamic_outer_price_distance_orderbook called")
        total_ask_volume = sum(float(ask[1]) for ask in order_book['asks'])
        total_bid_volume = sum(float(bid[1]) for bid in order_book['bids'])
    ...
    def adjust_distance_based_on_order_book(order_book, target_volume_percent):
        ...
        logging.info("Function adjust_distance_based_on_order_book called")
        """
    ...
    def adjust_distance_based_on_momentum(current_price, previous_prices, momentum_threshold=0.05):
        ...
        logging.info("Function adjust_distance_based_on_momentum called")
        """
    ...
    def calculate_orderbook_based_grid_levels(self, order_book, current_price, long_pos_price, short_pos_price, levels, max_outer_price_distance, min_buffer_percentage, max_buffer_percentage):
        ...
        logging.info("Function calculate_orderbook_based_grid_levels called")
        try:
            asks = order_book['asks']
            bids = order_book['bids']
            ...
            def volume_weighted_price(levels, side, buffer_percentage):
                ...
                logging.info("Function volume_weighted_price called")
                weighted_prices = []
                cumulative_volume = 0
    ...
    def get_best_prices(self, symbol, order_book, current_price):
        ...
        logging.info(f"[[{symbol}]] Function get_best_prices called")
    ...
    def calculate_buffers(self, symbol, current_price, long_pos_price, short_pos_price, long_pos_qty, short_pos_qty, initial_entry_buffer_pct, min_buffer_percentage, max_buffer_percentage, order_book):
        ...
        logging.info(f"[[{symbol}]] Function calculate_buffers called")
    ...
    def calculate_grid_levels_based_on_order_book(self, order_book, current_price, levels, strength, max_outer_price_distance, min_outer_price_distance):
        ...
        logging.info("Function calculate_grid_levels_based_on_order_book called")
        """
    ...
    def get_atrp(self, symbol: str, timeframe: str = '1m', period: int = 14, limit: int = 1000) -> float:
        ...
        logging.info(f"[[{symbol}]] Function get_atrp called")
        """
    ...
    def calculate_grid_levels_based_on_order_book_atr(self, atrp, order_book, current_price, levels, strength, max_outer_price_distance, min_outer_price_distance):
        ...
        logging.info("Function calculate_grid_levels_based_on_order_book_atr called")
        """
        Calculate grid levels based on order book data while ensuring levels stay within the specified outer price distance bounds.
        """
    ...
    def calculate_grid_levels_orderbook_based_volumes(self, symbol: str, current_price: float, buffer_distance: float, levels: int, side: str, min_outer_price_distance: float, max_outer_price_distance: float, strength: float):
        ...
        logging.info(f"[[{symbol}]] Function calculate_grid_levels_orderbook_based_volumes called")
        """
    ...
    def calculate_grid_levels_orderbook_based(self, symbol: str, current_price: float, buffer_distance: float, levels: int, side: str, min_outer_price_distance: float, max_outer_price_distance: float, strength: float):
        ...
        logging.info(f"[[{symbol}]] Function calculate_grid_levels_orderbook_based called")
        """
        Calculate grid levels based on the order book, ensuring levels stay within specified bounds.
    ...
    def get_30m_candle_spread(self, symbol: str, limit: int = 1) -> float:
        ...
        logging.info(f"[[{symbol}]] Function get_30m_candle_spread called")
    ...
    def get_4h_candle_spread(self, symbol: str) -> float:
        ...
        logging.info(f"[[{symbol}]] Function get_4h_candle_spread called")
    ...
    def get_position_qty(self, symbol, side):
        ...
        logging.info(f"[[{symbol}]] Function get_position_qty called")
    ...
    def log_total_grid_allocation(self, symbol: str, side: str, grid_levels: list, amounts: list):
        """
        Logs the total quantity and dollar value allocated to the grids.

        :param symbol: The trading symbol.
        :param side: 'Long' or 'Short'.
        :param grid_levels: List of grid level prices.
        :param amounts: List of order quantities corresponding to grid levels.
        """
        total_qty = sum(amounts)
        total_dollar = sum(qty * price for qty, price in zip(amounts, grid_levels))
        logging.info(
            f"[[{symbol}]] Total {side.capitalize()} Grid Allocation: "
            f"Quantity: {total_qty:.4f} units, "
    ...
    def lineargrid_base(
        ...
        levels: int,
        strength: float, 
        min_outer_price_distance: float, 
        max_outer_price_distance_long: float, 
        max_outer_price_distance_short: float, 
        reissue_threshold: float, 
        wallet_exposure_limit_long: float, 
        wallet_exposure_limit_short: float, 
        long_mode: bool,
        short_mode: bool, 
        initial_entry_buffer_pct: float, 
        min_buffer_percentage: float, 
        max_buffer_percentage: float, 
        symbols_allowed: int, 
        enforce_full_grid: bool, 
        mfirsi_signal: str, 
        upnl_profit_pct: float,
        max_upnl_profit_pct: float, 
        tp_order_counts: dict, 
        entry_during_autoreduce: bool,
        max_qty_percent_long: float, 
        max_qty_percent_short: float, 
        graceful_stop_long: bool, 
        graceful_stop_short: bool,
        additional_entries_from_signal: bool, 
        ...
        logging.info(f"[[{symbol}]] Calling function lineargrid_base (called by {caller_name})")
        
        # Log the full stack trace with proper formatting
        # formatted_stack = "".join(traceback.format_stack())
        # logging.info(f"[[{symbol}]] Calling function lineargrid_base. Call stack:\n{formatted_stack}")
        ...
        try:
            ...
            # Determine the current action based on running flags
            current_action = None
            if self.running_short and not self.running_long:
                current_action = "short"
            elif self.running_long and not self.running_short:
                current_action = "long"
            elif self.running_long and self.running_short:
                current_action = "both"
            else:
                logging.warning(f"[[{symbol}]] No active action specified. Skipping grid handling.")
                return  # Exit the method as no action is active

            logging.info(f"[[{symbol}]] Current action: {current_action}")

            # Conditionally handle grids based on current_action
            if current_action in ["long", "both"]:
                logging.info(f"[[{symbol}]] Handling long grid orders.")

                # Make the lists empty instead of filling them with zeros
                grid_levels_short_handled = []
                amounts_short_handled = []
                
                # Handle long grid trades
            ...
            if current_action in ["short", "both"]:
                logging.info(f"[[{symbol}]] Handling short grid orders.")

                # Make the lists empty instead of filling them with zeros
                grid_levels_long_handled = []
                amounts_long_handled = []
                
                # Handle short grid trades
                self.handle_grid_trades(
                    symbol,
                    grid_levels_long_handled,
                    grid_levels_short,
    ...
    def get_spread_and_price(self, symbol):
        ...
        logging.info(f"[[{symbol}]] Function get_spread_and_price called")
        spread = self.get_4h_candle_spread(symbol)
        logging.info(f"[[{symbol}]] 4h Candle spread for [[{symbol}]]: {spread}")
    ...
    def calculate_dynamic_outer_price_distance_latest(self, spread, min_outer_price_distance, max_outer_price_distance_long, max_outer_price_distance_short, symbol):

        logging.info(f"[[{symbol}]] Function calculate_dynamic_outer_price_distance_latest called")
        ...
        logging.info(f"[[{symbol}]] Dynamic outer price distance (Long): {dynamic_outer_price_distance_long}")
        logging.info(f"[[{symbol}]] Dynamic outer price distance (Short): {dynamic_outer_price_distance_short}")
        
        return dynamic_outer_price_distance_long, dynamic_outer_price_distance_short

    def initialize_filled_levels(self, symbol):

        logging.info(f"[[{symbol}]] Function initialize_filled_levels called")
    ...
    def check_grid_active(self, symbol, open_orders):
        # logging.info(f"[[{symbol}]] Function check_grid_active called")
        # logging.info(f"[[{symbol}]] Function issue_grid_safely called")
        # Automatically capture the caller name
        stack = inspect.stack()
        caller_name = stack[1].function

        # Log the caller name and formatted call stack
        logging.info(f"[[{symbol}]] Calling function check_grid_active (called by {caller_name})")
        
        # Log the full stack trace with proper formatting
        # formatted_stack = "".join(traceback.format_stack())
        # logging.info(f"[[{symbol}]] Calling function check_grid_active. Call stack:\n{formatted_stack}")


        logging.info(f"[[{symbol}]] Checking grid active for {symbol}")

        # Inspect each order's structure
        for order in open_orders:
            logging.debug(f"[[{symbol}]] Open Order: {order}")

        # Check for active long grid orders
        has_open_long_order = any(
            order.get('info', {}).get('symbol') == symbol and 
            order.get('info', {}).get('side', '').lower() == 'buy' and 
            not order.get('info', {}).get('reduceOnly', False)
            for order in open_orders
        )

        # Check for active short grid orders
        has_open_short_order = any(
            order.get('info', {}).get('symbol') == symbol and 
            order.get('info', {}).get('side', '').lower() == 'sell' and 
            not order.get('info', {}).get('reduceOnly', False)
            for order in open_orders
        )
        ...
        # Determine if grids are active based on active grids sets
        ...
        logging.info(f"[[{symbol}]] Grid Active Status - Long: {long_grid_active}, Short: {short_grid_active}")
    ...
    def calculate_buffer_percentages(self, long_pos_qty, short_pos_qty, current_price, long_pos_price, short_pos_price, initial_entry_buffer_pct, min_buffer_percentage, max_buffer_percentage):
        ...
        logging.info("Function calculate_buffer_percentages called")
    ...
    def calculate_buffer_distances(self, current_price, buffer_percentage_long, buffer_percentage_short, symbol):

        logging.info(f"[[{symbol}]] Function calculate_buffer_distances called")
    ...
    def get_order_book_prices(self, symbol, current_price):
        ...
        logging.info(f"[[{symbol}]] Function get_order_book_prices called")
        order_book = self.exchange.get_orderbook(symbol)
        best_ask_price = order_book['asks'][0][0] if 'asks' in order_book else self.last_known_ask.get(symbol, current_price)
        best_bid_price = order_book['bids'][0][0] if 'bids' in order_book else self.last_known_bid.get(symbol, current_price)
    ...
    def calculate_price_range_and_volume_histograms(self, order_book, current_price, max_outer_price_distance_long, max_outer_price_distance_short):
        ...
        logging.info("Function calculate_price_range_and_volume_histograms called")
        try:
    ...
    def calculate_volume_thresholds_and_significant_levels(self, volume_histogram, price_range):
        ...
        logging.info("Function calculate_volume_thresholds_and_significant_levels called")
    ...
    def calculate_initial_entries(self, current_price, buffer_distance_long, buffer_distance_short):
        ...
        logging.info("Function calculate_initial_entries called")
    ...
    def calculate_grid_levels(self, long_pos_qty, short_pos_qty, levels, initial_entry_long, initial_entry_short, 
                            current_price, buffer_distance_long, buffer_distance_short, 
                            max_outer_price_distance_long, max_outer_price_distance_short):
        logging.info("Function calculate_grid_levels called")
    ...
    def adjust_grid_levels(self, grid_levels, significant_levels, tolerance, min_outer_price_distance, max_outer_price_distance_long, max_outer_price_distance_short, current_price, levels):
        ...
        logging.info("Function adjust_grid_levels called")
        def find_nearest_significant_level(level, significant_levels, tolerance, min_distance, max_distance, current_price, previous_level=None):
            ...
            logging.info("Function find_nearest_significant_level called")
    ...
    def finalize_grid_levels(
        self, 
        adjusted_grid_levels_long, 
        adjusted_grid_levels_short, 
        levels, 
        current_price, 
        buffer_distance_long, 
        buffer_distance_short, 
        ...
        logging.info(f"[[{symbol}]] Function finalize_grid_levels called")
        ...
        logging.info(f"[[{symbol}]] Function: finalize_grid_levels")
        # Format initial entry levels to 4 decimal places
        logging.info(f"[[{symbol}]] Initial long entry level: {initial_entry_long:.4f}")
        logging.info(f"[[{symbol}]] Initial short entry level: {initial_entry_short:.4f}")

        # Format grid levels to 4 decimal places
        formatted_long_levels = [f"{level:.4f}" for level in adjusted_grid_levels_long]
        formatted_short_levels = [f"{level:.4f}" for level in adjusted_grid_levels_short]

        logging.info(f"[[{symbol}]] Long grid levels: {formatted_long_levels}")
        logging.info(f"[[{symbol}]] Short grid levels: {formatted_short_levels}")

        return adjusted_grid_levels_long, adjusted_grid_levels_short

    def get_precision_and_min_qty(self, symbol):

        logging.info(f"[[{symbol}]] Function get_precision_and_min_qty called")
        qty_precision = self.exchange.get_symbol_precision_bybit(symbol)[1]
    ...
    def calculate_total_amount_refactor(self, symbol, total_equity, best_ask_price, best_bid_price, 
        ...
        logging.info(f"[[{symbol}]] Function calculate_total_amount_refactor called")
    ...
    def calculate_order_amounts_refactor(self, symbol, total_amount, levels, strength, qty_precision, enforce_full_grid, long_pos_qty, short_pos_qty, side):
        ...
        logging.info(f"[[{symbol}]] Function calculate_order_amounts_refactor called")
    ...
    def issue_reduce_only_order(self, exchange, symbol, side, position_qty, stop_loss_pct, current_price, positionIdx):
        ...
        logging.info(f"[[{symbol}]] Function issue_reduce_only_order called")
        """
    ...
    def trigger_stop_loss(self, symbol, pos_qty, side, stop_loss_price, best_price):
        ...
        logging.info(f"[[{symbol}]] Function trigger_stop_loss called")
        try:
    ...
    def has_active_grid(self, symbol, side, open_orders):
        ...
        logging.info(f"[[{symbol}]] Function has_active_grid called")
    ...
    def issue_grid_safely(self, symbol: str, side: str, grid_levels: List[float], amounts: List[float], open_orders: List[dict]):
        ...
        # Log the caller name and formatted call stack
        logging.info(f"[[{symbol}]] Calling function issue_grid_safely (called by {caller_name})")
        
        # Log the full stack trace with proper formatting
        # formatted_stack = "".join(traceback.format_stack())
        # logging.info(f"[[{symbol}]] Calling function issue_grid_safely. Call stack:\n{formatted_stack}")

        with self.grid_lock:  # Lock to ensure no simultaneous grid issuance
            try:
                ...
                if self.grid_cleared_status.get(symbol, {}).get(side, False):
                    ...
                    # Enhanced Logging: Log each order placed
                    for i, (price, qty) in enumerate(zip(grid_levels, amounts), 1):
                        dollar_value = qty * price
                        logging.info(
                            f"[[{symbol}]] Issued {side} Grid Order - Level {i}: "
                            f"Price: {price:.4f} USD, Quantity: {qty:.4f} units, Dollar Value: ${dollar_value:.4f} USD"
                        )
                ...
                logging.error(f"[[{symbol}]] Exception in issue_grid_safely for {symbol} - {side}: {e}")
    ...
    def handle_grid_trades(
        self, 
        symbol: str, 
        grid_levels_long: List[float], 
        grid_levels_short: List[float], 
        long_grid_active: bool, 
        short_grid_active: bool,
        long_pos_qty: float, 
        short_pos_qty: float, 
        current_price: float, 
        ...
        min_outer_price_distance: float, 
        buffer_percentage_long: float, 
        buffer_percentage_short: float, 
        adjusted_grid_levels_long: List[float], 
        adjusted_grid_levels_short: List[float], 
        levels: int, 
        amounts_long: List[float], 
        amounts_short: List[float], 
        ...
        """
        Handles grid trades for a given symbol by processing either long or short grids
        based on the passed grid levels and amounts. This method ensures that only the
        active grid side is processed, preventing unintended order placements.
        ...
        try:
            ...
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) (called by {caller_name})")
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Function called with grid_levels_long: {grid_levels_long} and grid_levels_short: {grid_levels_short}")
            
            # Determine if processing long or short grid based on grid_levels and amounts
            is_long = any(qty > 0 for qty in amounts_long)
            is_short = any(qty > 0 for qty in amounts_short)
            
            logging.info(f"[[{symbol}]] Processing {'long' if is_long else 'short' if is_short else 'none'} grid trades.")
            
            # Process Long Grid Trades
            if is_long:
                logging.info(f"[[{symbol}]] Processing long grid trades.")
            filled_levels_long = set()
            self.issue_grid_orders(
                symbol=symbol,
                side="buy",
                grid_levels=grid_levels_long,
                amounts=amounts_long,
                is_long=True,
                filled_levels=filled_levels_long
            )
            logging.info(f"[[{symbol}]] Placed long grid orders.")
            
            # Process Short Grid Trades
            if is_short:
                logging.info(f"[[{symbol}]] Processing short grid trades.")
            filled_levels_short = set()
            self.issue_grid_orders(
                symbol=symbol,
                side="sell",
                grid_levels=grid_levels_short,
                amounts=amounts_short,
                is_long=False,
                filled_levels=filled_levels_short
            )
            logging.info(f"[[{symbol}]] Placed short grid orders.")
            
            # Log Current Position Quantities
            logging.info(f"[[{symbol}]] Long Position Quantity: {long_pos_qty}")
            logging.info(f"[[{symbol}]] Short Position Quantity: {short_pos_qty}")
            
            # Initialize previous_position_state if not present
            ...
            # Determine if positions have been closed
            ...
            if long_position_closed:
                logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Long position closed. Resetting initial entry price for long.")
                self.previous_position_state[symbol]['long_initial_entry'] = None
            
            if short_position_closed:
                logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Short position closed. Resetting initial entry price for short.")
            ...
            if has_open_long_position and not self.previous_position_state[symbol]['long_initial_entry']:
                ...
                logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Long position opened. Recording initial entry price for stop-loss: {long_pos_price:.4f})")
            ...
            if has_open_short_position and not self.previous_position_state[symbol]['short_initial_entry']:
                ...
                logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Short position opened. Recording initial entry price for stop-loss: {short_pos_price:.4f})")
            ...
            if stop_loss_enabled:
                ...
                logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Calculated stop-loss prices: Long - {stop_loss_price_long:.4f}, Short - {stop_loss_price_short:.4f})")

                # Long position stop-loss check
                if has_open_long_position:
                    logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Long Position Quantity: {long_pos_qty:.4f}, Entry Price: {long_pos_price:.4f}, Stop-Loss Price: {stop_loss_price_long:.4f})")
                    if current_price <= stop_loss_price_long:
                        logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Long position hit stop-loss at {stop_loss_price_long:.4f}. Triggering stop-loss.)")
                ...
                if has_open_short_position:
                    logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Short Position Quantity: {short_pos_qty:.4f}, Entry Price: {short_pos_price:.4f}, Stop-Loss Price: {stop_loss_price_short:.4f})")
                    if current_price >= stop_loss_price_short:
                        logging.info(f"[[{symbol}]]func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Short position hit stop-loss at {stop_loss_price_short:.4f}. Triggering stop-loss.)")
                        self.trigger_stop_loss(symbol, short_pos_qty, 'short', stop_loss_price_short, best_ask_price)
            else:
                logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Stop-loss disabled")
            ...
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Open symbols long: {open_symbols_long}")
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Open symbols short: {open_symbols_short}")
            ...
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Length of open symbols long: {length_of_open_symbols_long}")
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Length of open symbols short: {length_of_open_symbols_short}")
            ...
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Unique open symbols: {unique_open_symbols}")
            ...
            if self.auto_reduce_active_long.get(symbol, False):
                logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Auto-reduce for long position on [[{symbol}]] is active")
                self.clear_grid(symbol, 'buy')
                #self.active_long_grids.discard(symbol)
            else:
                logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Auto-reduce for long position on [[{symbol}]] is not active")

            if self.auto_reduce_active_short.get(symbol, False):
                logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Auto-reduce for short position on [[{symbol}]] is active")
                self.clear_grid(symbol, 'sell')
                #self.active_short_grids.discard(symbol)
            else:
                logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Auto-reduce for short position on [[{symbol}]] is not active")
            ...
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Symbol format: {symbol}")
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Test open orders: {open_orders}")
            has_open_long_order = any(
                order['info']['symbol'] == symbol and 
                order['info']['side'].lower() == 'buy' and 
                not order['info']['reduceOnly'] 
                for order in open_orders
            )
            has_open_short_order = any(
                order['info']['symbol'] == symbol and 
                order['info']['side'].lower() == 'sell' and 
                not order['info']['reduceOnly'] 
                for order in open_orders
            )

            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) {symbol} Has open long order: {has_open_long_order}")
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) {symbol} Has open short order: {has_open_short_order}")
            ...
            # Log if the symbol is in max_qty_reached_symbol_long or short
            ...
            # Combine open symbols and check trading permissions

            # Additional logic for managing open symbols and checking trading permissions
            open_symbols = list(set(all_open_symbols))
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Open symbols: {open_symbols}")

            trading_allowed = self.can_trade_new_symbol(self.symbols_allowed, symbol)
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Checking trading for symbol [[{symbol}]]. Can trade: {trading_allowed}")
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Symbol: [[{symbol}]], In open_symbols: {symbol in open_symbols}, Trading allowed: {trading_allowed}")
            ...
            if unique_open_symbols <= symbols_allowed or symbol in open_symbols:
                ...
                try:
                    ...
                    if (
                        ...
                        logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Creating new long position based on MFIRSI long signal")
                        ...
                        if not self.has_active_grid(symbol, 'long', open_orders):
                            ...
                            if not has_open_long_position:
                                ...
                                logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Setting first level of modified grid to best_bid_price: {best_bid_price:.4f} USD")
                                modified_grid_levels_long[0] = best_bid_price

                            # Log the parameters
                            logging.info(
                                f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Calling function issue_grid_safely with params: symbol=%s, side=%s, grid_levels=%s, amounts=%s, open_orders=%s",
                                symbol, 'long', modified_grid_levels_long, amounts_long, open_orders
                            )   
                            # Issue the grid orders
                            logging.info(
                                f"[[{symbol}]] Calling issue_grid_safely with params: symbol={symbol}, side=long, grid_levels={modified_grid_levels_long}, amounts={amounts_long}, open_orders={open_orders}"
                            )
                            ...
                            while long_pos_qty < 0.00001 and retry_counter < max_retries:
                                ...
                                try:
                                    ...
                                    logging.error(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Error fetching long position quantity: {e}")
                                    break

                                retry_counter += 1
                                logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Long position quantity after waiting: {long_pos_qty:.6f}, retry attempt: {retry_counter}")

                                # Retry placing the grid every 10 retries (every 30 seconds)
                                if retry_counter % 10 == 0 and long_pos_qty < 0.00001:
                                    logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Retrying long grid orders due to MFIRSI signal long after {retry_counter} retries.")

                                    # Fetch the latest best bid price before retrying
                                    best_bid_price = self.get_best_bid_price(symbol)
                                    logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Updated best bid price on retry: {best_bid_price:.4f} USD")
                            ...
                            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Long position filled or max retries reached, exiting loop.")
                    ...
                    # Handling for Short Positions
                        ...
                        logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Creating new short position based on MFIRSI short signal")
                        ...
                        if not self.has_active_grid(symbol, 'short', open_orders):
                            ...
                            if not has_open_short_position:
                                ...
                                logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Setting first level of modified grid to best_ask_price: {best_ask_price:.4f} USD")
                                modified_grid_levels_short[0] = best_ask_price

                            # Log the parameters
                            logging.info(
                                f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Calling function issue_grid_safely with params: symbol=%s, side=%s, grid_levels=%s, amounts=%s, open_orders=%s",
                                symbol, 'short', modified_grid_levels_short, amounts_short, open_orders
                            )
                            # Issue the grid orders
                            logging.info(
                                f"[[{symbol}]] Calling issue_grid_safely with params: symbol={symbol}, side=short, grid_levels={modified_grid_levels_short}, amounts={amounts_short}, open_orders={open_orders}"
                            )
                            ...
                            while short_pos_qty < 0.00001 and retry_counter < max_retries:
                                ...
                                try:
                                    ...
                                    logging.error(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Error fetching short position quantity: {e}")
                                    break

                                retry_counter += 1
                                logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Short position quantity after waiting: {short_pos_qty:.6f}, retry attempt: {retry_counter}")

                                # Retry placing the grid every 10 retries (every 50 seconds)
                                if retry_counter % 10 == 0 and short_pos_qty < 0.00001:
                                    logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Retrying short grid orders due to MFIRSI signal short after {retry_counter} retries.")
                                    
                                    # Fetch the latest best ask price before retrying
                                    best_ask_price = self.get_best_ask_price(symbol)
                                    logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Updated best ask price on retry: {best_ask_price:.4f} USD")
                                    ...
                                    # Log the parameters
                                    logging.info(
                                        f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Calling function issue_grid_safely with params: symbol=%s, side=%s, grid_levels=%s, amounts=%s, open_orders=%s (line: %d)",
                                        symbol, 'long', modified_grid_levels_long, amounts_long, open_orders, inspect.currentframe().f_lineno
                                    )
                                   
                                    self.issue_grid_safely(symbol, 'short', modified_grid_levels_short, amounts_short, open_orders)

                            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Short position filled or max retries reached, exiting loop.")
                            self.last_signal_time[symbol] = current_time
                            self.last_mfirsi_signal[symbol] = "neutral"  # Reset to neutral after processing

                except Exception as e:
                    logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Exception caught in placing orders: {e}")
            ...
            if additional_entries_from_signal:
                if symbol in open_symbols:
                    logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Allowed symbol: [[{symbol}]]")

                    fresh_signal = self.generate_l_signals(symbol)

                    logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Fresh signal for [[{symbol}]] : {fresh_signal}")
                    ...
                    if time_since_last_signal < 180:  # 3 minutes
                        logging.info(
                            f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Waiting for signal cooldown. Time since last signal: {time_since_last_signal:.2f} seconds"
                        )
                        return

                    if fresh_signal.lower() != self.last_mfirsi_signal[symbol]:
                        logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) MFIRSI signal changed to {fresh_signal}")
                        self.last_mfirsi_signal[symbol] = fresh_signal.lower()
                    else:
                        logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) MFIRSI signal unchanged: {fresh_signal}")
                    ...
                    try:
                        ...
                        if (
                            fresh_signal.lower() == "long" and 
                            long_mode and 
                            not self.auto_reduce_active_long.get(symbol, False) and 
                            long_pos_qty > 0.00001 and 
                            symbol not in self.max_qty_reached_symbol_long
                        ):
                            if current_price <= long_pos_price:  # Enter additional entry only if current price <= long_pos_price
                                logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Adding to existing long position based on MFIRSI long signal" )
                                ...
                                if not self.has_active_grid(symbol, 'long', open_orders):
                                    ...
                                    while long_pos_qty < 0.00001 and retry_counter < max_retries:
                                        ...
                                        retry_counter += 1
                                        logging.info(
                                            f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Long position quantity after retry: {long_pos_qty:.6f}, retry attempt: {retry_counter}"
                                        )

                                        if retry_counter % 10 == 0 and long_pos_qty < 0.00001:
                                            logging.info(
                                                f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Retrying long grid orders."
                                            )
                                            # Fetch the latest best bid price before retrying
                                            best_bid_price = self.get_best_bid_price(symbol)
                                            logging.info(
                                                f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Updated best bid price on retry: {best_bid_price:.4f} USD"
                                            )
                                            ...
                                            logging.info(
                                                f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Long position filled or max retries reached, exiting loop."
                                            )
                                            break
                        ...
                        # Handling for additional Short entries
                            ...
                            fresh_signal.lower() == "short" and 
                            short_mode and 
                            not self.auto_reduce_active_short.get(symbol, False) and 
                            short_pos_qty > 0.00001 and 
                            symbol not in self.max_qty_reached_symbol_short
                        ):
                            if current_price >= short_pos_price:  # Enter additional entry only if current price >= short_pos_price
                                logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Adding to existing short position based on MFIRSI short signal")
                                ...
                                if not self.has_active_grid(symbol, 'short', open_orders):
                                    ...
                                    while short_pos_qty < 0.00001 and retry_counter < max_retries:
                                        ...
                                        retry_counter += 1
                                        logging.info(
                                            f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Short position quantity after retry: {short_pos_qty:.6f}, retry attempt: {retry_counter}"
                                        )

                                        if retry_counter % 10 == 0 and short_pos_qty < 0.00001:
                                            logging.info(
                                                f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Retrying short grid orders."
                                            )
                                            # Fetch the latest best ask price before retrying
                                            best_ask_price = self.get_best_ask_price(symbol)
                                            logging.info(
                                                f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Updated best ask price on retry: {best_ask_price:.4f} USD"
                                            )
                                            ...
                                            logging.info(
                                                f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Short position filled or max retries reached, exiting loop."
                                            )
                                            break
                            ...
                            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) MFIRSI signal is neutral. No new grid orders.")
            ...
            # Re-check grid active status
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Symbol type for grid active check: {symbol}")
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Calling function check_grid_active (called by handle_grid_trades)")
            long_grid_active, short_grid_active = self.check_grid_active(symbol, open_orders)

            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Updated long grid active: {long_grid_active}")
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Updated short grid active: {short_grid_active}")
            ...
            #     self.active_long_grids.discard(symbol)
            #     logging.info(f"[[{symbol}]]handle_grid_trades  No open long orders, removed from active long grids")

            # if not has_open_short_order and symbol in self.active_short_grids:
            #     self.active_short_grids.discard(symbol)
            #     logging.info(f"[[{symbol}]]handle_grid_trades  No open short orders, removed from active short grids")

            # Check if the symbol is in active grids without open orders

            # Log the current status of open long and short orders for the symbol
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) has_open_long_order: {has_open_long_order}")
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) has_open_short_order: {has_open_short_order}")

            # Handle active long grids
            if not has_open_long_order and symbol in self.active_long_grids:
                self.active_long_grids.discard(symbol)
                logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) No open long orders, removed from active long grids")
            else:
                if symbol in self.active_long_grids:
                    logging.debug(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Open long orders exist, keeping in active long grids")
                else:
                    logging.debug(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Symbol not present in active long grids")

            # Handle active short grids
            if not has_open_short_order and symbol in self.active_short_grids:
                self.active_short_grids.discard(symbol)
                logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) No open short orders, removed from active short grids")
            else:
                if symbol in self.active_short_grids:
                    logging.debug(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Open short orders exist, keeping in active short grids")
                else:
                    logging.debug(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Symbol not present in active short grids")


            """
            Handles grid trades for a given symbol by checking the status of open positions
            and determining if new grid orders need to be issued.

            Checks:
            1. If the `symbol` is currently in the list of `open_symbols`.
            2. If the symbol has:
                a. A long position (`long_pos_qty > 0`), and:
                    - No active long grid (`not long_grid_active`), and
                    - No active grid orders for the long side (`not self.has_active_grid(symbol, 'long', open_orders)`).
                b. OR a short position (`short_pos_qty > 0`), and:
                    - No active short grid (`not short_grid_active`), and
                    - No active grid orders for the short side (`not self.has_active_grid(symbol, 'short', open_orders)`).

            If all conditions for either the long side or the short side are met, the function logs the event and issues grid orders for the symbol.

            Args:
                symbol (str): The trading pair symbol (e.g., BTCUSDT).
                open_symbols (list): List of symbols currently being tracked as open.
                long_pos_qty (float): Quantity of the long position for the symbol.
                short_pos_qty (float): Quantity of the short position for the symbol.
                long_grid_active (bool): Indicates if the long grid is active for the symbol.
                short_grid_active (bool): Indicates if the short grid is active for the symbol.
                open_orders (list): List of currently open orders for the symbol.

            Logs:
                If all conditions for either the long side or the short side are met (e.g., open positions exist without 
                corresponding active grids), logs this event and proceeds to issue grid orders.
            """
            ...
            if symbol in open_symbols: 
                if (
                    ...
                    logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Open positions found without active grids. Issuing grid orders.")
                    ...
                    if long_pos_qty > 0 and not long_grid_active and symbol not in self.max_qty_reached_symbol_long:
                        if not self.auto_reduce_active_long.get(symbol, False) or entry_during_autoreduce:
                            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Placing long grid orders for existing open position.")
                            ...
                            modified_grid_levels_long = grid_levels_long.copy()
                            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Modified grid levels long: {modified_grid_levels_long}")
                            ...
                            # logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Setting first level of modified long grid to best_bid_price - 0.5%: {modified_grid_levels_long[0]}")
                            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Not modifying long_grid_levels, keeping original: {modified_grid_levels_long}")

                            formatted_levels = ", ".join([f"{level:.2f}" for level in modified_grid_levels_long])
                            logging.info(
                                f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Modified long grid levels: [{formatted_levels}]"
                            )
                            # Issue long grid safely
                            # Log the parameters
                            logging.info(
                                f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Calling issue_grid_safely with params BLANK: symbol=%s, side=%s, grid_levels=%s, amounts=%s, open_orders=%s" ,
                                symbol, 'long', modified_grid_levels_long, amounts_long, open_orders
                            )
                            self.issue_grid_safely(symbol, 'long', modified_grid_levels_long, amounts_long, open_orders)
                            # pass
                    ...
                    if short_pos_qty > 0 and not short_grid_active and symbol not in self.max_qty_reached_symbol_short:
                        if not self.auto_reduce_active_short.get(symbol, False) or entry_during_autoreduce:
                            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Placing short grid orders for existing open position.")
                            ...
                            # logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Setting first level of modified short grid to best_ask_price + 0.5%: {modified_grid_levels_short[0]}")
                            
                            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Not modifying short_grid_levels, keeping original: {modified_grid_levels_short}")

                            formatted_levels = ", ".join([f"{level:.2f}" for level in modified_grid_levels_short])
                            logging.info(
                                f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Modified short grid levels: [{formatted_levels}]"
                ...
                if not long_pos_qty and not short_pos_qty and symbol in self.active_long_grids | self.active_short_grids:
                    ...
                    if current_time - last_cleared > self.clear_interval:
                        logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) No open positions and time interval passed. Canceling leftover grid orders.")
                        self.clear_grid(symbol, 'buy')
                        self.clear_grid(symbol, 'sell')
                        self.last_empty_grid_time[symbol] = current_time
                    else:
                        logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) No open positions, but time interval not passed. Skipping grid clearing.")
            ...
            # Update Take Profit (TP) for short position
            if short_pos_qty > 0:
            ...
            if has_open_long_order and (long_pos_price is None or long_pos_price <= 0) and not mfi_signal_long:
                if symbol not in self.max_qty_reached_symbol_long:
                    ...
                    if symbol not in self.invalid_long_condition_time:
                        ...
                        logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Invalid long condition first met for {symbol}. Waiting before clearing grid...")
                    ...
                    # If 1 minute has passed since the condition was first encountered
                        ...
                        self.clear_grid(symbol, "buy")
                        logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Cleared long grid for {symbol} after 1 minute of invalid long_pos_price: {long_pos_price:.4f} and no long signal.")
                        # Reset the tracking time after clearing
                        del self.invalid_long_condition_time[symbol]
                else:
                    logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) is in max qty reached symbol long, cannot replace grid")
            else:
            ...
            if has_open_short_order and (short_pos_price is None or short_pos_price <= 0) and not mfi_signal_short:
                if symbol not in self.max_qty_reached_symbol_short:
                    ...
                    if symbol not in self.invalid_short_condition_time:
                        ...
                        logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Invalid short condition first met for {symbol}. Waiting before clearing grid...")

                    # If 1 minute has passed since the condition was first encountered
                    elif current_time - self.invalid_short_condition_time[symbol] >= 60:
                        self.clear_grid(symbol, "sell")
                        logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Cleared short grid for {symbol} after 1 minute of invalid short_pos_price: {short_pos_price:.4f} and no short signal.")
                        # Reset the tracking time after clearing
                        del self.invalid_short_condition_time[symbol]
                else:
                    logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) is in max qty reached symbol short, cannot replace grid")
            else:
                # Reset the tracking if conditions are no longer met
            ...
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Error in executing gridstrategy: {e}")
            logging.info(f"[[{symbol}]] func:{inspect.currentframe().f_code.co_name} (line: {inspect.currentframe().f_lineno}) Traceback: %s", traceback.format_exc())
    ...
    def linear_grid_hardened_gridspan(
        ...
        logging.info(f"[[{symbol}]] Function linear_grid_hardened_gridspan called")
    ...
    def adjust_outer_price_distance(self, current_price, buffer_distance, levels, strength):
        ...
        logging.info("Function adjust_outer_price_distance called")
    ...
    def linear_grid_dynamictp_linspaced_maxposqty(
        ...
        logging.info(f"[[{symbol}]] Function linear_grid_dynamictp_linspaced_maxposqty called")
        try:
            should_reissue_long, should_reissue_short = self.should_reissue_orders_revised(
                symbol, reissue_threshold, long_pos_qty, short_pos_qty, initial_entry_buffer_pct)
            open_orders = self.retry_api_call(self.exchange.get_open_orders, symbol)
    ...
    def linear_grid_handle_positions(self, symbol: str, open_symbols: list, total_equity: float, long_pos_qty: float, short_pos_qty: float, levels: int, strength: float, outer_price_distance: float, reissue_threshold: float, wallet_exposure_limit: float, user_defined_leverage_long: float, user_defined_leverage_short: float, long_mode: bool, short_mode: bool, buffer_percentage: float, symbols_allowed: int, enforce_full_grid: bool):
        ...
        logging.info(f"[[{symbol}]] Function linear_grid_handle_positions called")
        try:
            if symbol not in self.symbol_locks:
                self.symbol_locks[symbol] = threading.Lock()
    ...
    def should_replace_grid_updated_buffer_dynamic(self, symbol: str, long_pos_price: float, short_pos_price: float, long_pos_qty: float, short_pos_qty: float, min_buffer_percentage: float, max_buffer_percentage: float) -> tuple:
        ...
        logging.info(f"[[{symbol}]] Function should_replace_grid_updated_buffer_dynamic called")
        try:
            current_price = self.exchange.get_current_price(symbol)
    ...
    def should_replace_grid_updated_buffer(self, symbol: str, long_pos_price: float, short_pos_price: float, long_pos_qty: float, short_pos_qty: float, min_buffer_percentage: float, max_buffer_percentage: float) -> tuple:
        ...
        logging.info(f"[[{symbol}]] Function should_replace_grid_updated_buffer called")
        try:
            current_price = self.exchange.get_current_price(symbol)
            logging.info(f"[[{symbol}]] Current price: {current_price}")
    ...
    def should_replace_grid_updated_buffer_min_outerpricedist_v2(
        ...
        logging.info(f"[[{symbol}]] Function should_replace_grid_updated_buffer_min_outerpricedist_v2 called")
        ...
        try:
            ...
            if long_pos_qty > 0 and last_reissue_price_long is not None:
                ...
                logging.info(f"[[{symbol}]] Long position info:")
                logging.info(f"[[{symbol}]] Dynamic outer price distance: {dynamic_outer_price_distance_long * 100.0:.2f}%")
                logging.info(f"[[{symbol}]]  - Long position price: {long_pos_price}")
                logging.info(f"[[{symbol}]]  - Long position quantity: {long_pos_qty}")
            ...
            if short_pos_qty > 0 and last_reissue_price_short is not None:
                ...
                logging.info(f"[[{symbol}]] Short position info:")
                logging.info(f"[[{symbol}]] Dynamic outer price distance: {dynamic_outer_price_distance_short * 100.0:.2f}%")
                logging.info(f"[[{symbol}]]  - Short position price: {short_pos_price}")
                logging.info(f"[[{symbol}]]  - Short position quantity: {short_pos_qty}")
    ...
    def should_replace_grid_updated_buffer_min_outerpricedist(self, symbol: str, long_pos_price: float, short_pos_price: float, long_pos_qty: float, short_pos_qty: float, min_outer_price_distance: float) -> tuple:
        ...
        logging.info(f"[[{symbol}]] Function should_replace_grid_updated_buffer_min_outerpricedist called")
        try:
            current_price = self.exchange.get_current_price(symbol)
            logging.info(f"[[{symbol}]] Current price: {current_price}")
    ...
    def should_reissue_orders_revised(self, symbol: str, reissue_threshold: float, long_pos_qty: float, short_pos_qty: float, initial_entry_buffer_pct: float) -> tuple:
        ...
        logging.info(f"[[{symbol}]] Function should_reissue_orders_revised called")
        try:
            current_price = self.exchange.get_current_price(symbol)
            last_price = self.last_price.get(symbol)
    ...
    def should_reissue_orders(self, symbol: str, reissue_threshold: float) -> bool:
        ...
        logging.info(f"[[{symbol}]] Function should_reissue_orders called")
        try:
            current_price = self.exchange.get_current_price(symbol)
            last_price = self.last_price.get(symbol)
    ...
    def clear_grid(self, symbol, side, max_retries=50, delay=2):
        ...
        logging.info(f"[[{symbol}]] Calling function clear_grid (called by {caller_name}). Clearing {side} grid for {symbol}")
        # Log the full stack trace with proper formatting
        # formatted_stack = "".join(traceback.format_stack())
        # logging.info(f"[[{symbol}]] Calling function clear_grid. Call stack:\n{formatted_stack}")

        # logging.info(f"[[{symbol}]] Function clear_grid called")
        """Clear all orders and internal states for a specific grid side with retries, excluding reduce-only orders."""
        # logging.info(f"[[{symbol}]] Calling function clear_grid Clearing {side} grid for {symbol}")
        ...
        while retry_counter < max_retries:
            ...
            if not lingering_orders:
                ...
                logging.info(f"[[{symbol}]] Successfully cleared {side} grid for {symbol}.")
                ...
                logging.warning(f"[[{symbol}]] Lingering {side} orders detected for {symbol} (non-reduce-only): {lingering_orders}. Retrying...")
                retry_counter += 1
                time.sleep(delay)  # Wait before retrying

        if retry_counter == max_retries:
            logging.error(f"[[{symbol}]] Failed to clear all {side} grid orders for {symbol} after {max_retries} attempts.")
        else:
            logging.info(f"[[{symbol}]] {side.capitalize()} grid cleared after {retry_counter} retries for {symbol}.")

    

    def generate_order_link_id(self, symbol, side, level):

        logging.info(f"[[{symbol}]] Function generate_order_link_id called")
        """
    ...
    def issue_grid_orders(self, symbol: str, side: str, grid_levels: list, amounts: list, is_long: bool, filled_levels: set):
        ...
        logging.info(f"[[{symbol}]] Calling function issue_grid_orders (called by {caller_name}).")
        logging.info(f"[[{symbol}]] Values passed to function issue_grid_orders: {grid_levels} and amounts: {amounts}")
        """
        ...
        try:
            ...
            if is_long:
                logging.info(f"[[{symbol}]] Clearing existing long grid for symbol before issuing new orders. (line: {inspect.currentframe().f_lineno})")
                self.clear_grid(symbol, 'buy')
                self.last_reissue_price_long[symbol] = current_price
                logging.info(f"[[{symbol}]] Updated last reissue price for long orders of symbol to {current_price}")
            else:
                logging.info(f"[[{symbol}]] Clearing existing short grid for symbol before issuing new orders. (line: {inspect.currentframe().f_lineno})")
                self.clear_grid(symbol, 'sell')
                self.last_reissue_price_short[symbol] = current_price
                logging.info(f"[[{symbol}]] Updated last reissue price for short orders of symbol to {current_price}")

            # Clear the filled_levels set before placing new orders
            filled_levels.clear()

            # Add logging to verify types before the zip operation
            logging.info(f"[[{symbol}]] Inside issue_grid_orders - Type of grid_levels: {type(grid_levels)}, Value: {grid_levels}")
            logging.info(f"[[{symbol}]] Inside issue_grid_orders - Type of amounts: {type(amounts)}, Value: {amounts}")
            ...
            for level, amount in zip(grid_levels, amounts):
                ...
                if not order_exists:
                    ...
                    # Log caller name before placing the order
                    logging.info(f"[[{symbol}]] Placing order at level {level} for {side} (called by {caller_name}).")
                    try:
                        logging.info(f"[[{symbol}]] Placing order: {side.capitalize()} at level {level:.4f} with order_link_id {order_link_id} and amount {amount:.4f}")
                        ...
                        if order and 'id' in order:
                            ...
                            # logging.info(f"[[{symbol}]] Placed order: {side.capitalize()} at level {level:.4f} with order_link_id {order_link_id} and amount {amount:.4f}")
                            filled_levels.add(level)  # Add the level to filled_levels
                        else:
                            logging.info(f"[[{symbol}]] Failed to place {side} order at level {level} for symbol with amount {amount}")
                    except Exception as e:
                        logging.error(f"[[{symbol}]] Exception when placing {side} order at level {level} for symbol: {e}")
                else:
    ...
    def cancel_grid_orders(self, symbol: str, side: str):
        ...
        logging.info(f"[[{symbol}]] Function cancel_grid_orders called")
        try:
            open_orders = self.retry_api_call(self.exchange.get_open_orders, symbol)
            ...
            for order in open_orders:
                ...
                if order['side'].lower() == side.lower() and not order.get('reduceOnly', False):
                    self.exchange.cancel_order_by_id(order['id'], symbol)
                    orders_canceled += 1
                    logging.info(f"[[{symbol}]] Canceled order {order['id']} for [[{symbol}]]")

            if orders_canceled > 0:
                logging.info(f"[[{symbol}]] Canceled {orders_canceled} {side} grid orders for [[{symbol}]]")
            else:
                logging.info(f"[[{symbol}]] No {side} grid orders found to cancel for {symbol}")

            # Remove the symbol from active grids
            if side.lower() == 'buy' and orders_canceled > 0:
                self.active_long_grids.discard(symbol)
                logging.info(f"[[{symbol}]] Removed symbol from active long grids")
            elif side.lower() == 'sell' and orders_canceled > 0:
                self.active_short_grids.discard(symbol)
                logging.info(f"[[{symbol}]] Removed symbol from active short grids")

        except Exception as e:
            logging.error(f"[[{symbol}]] An error occurred while canceling grid orders: {e}")
    ...
    def calculate_total_amount(self, symbol: str, total_equity: float, best_ask_price: float, best_bid_price: float, wallet_exposure_limit: float, user_defined_leverage: float, side: str, levels: int, min_qty: float, enforce_full_grid: bool) -> float:
        ...
        logging.info(f"[[{symbol}]] Function calculate_total_amount called")
    ...
    def calculate_order_amounts(self, symbol: str, total_amount: float, levels: int, strength: float, qty_precision: float, min_qty: float, enforce_full_grid: bool) -> List[float]:
        ...
        logging.info(f"[[{symbol}]] Function calculate_order_amounts called")
    ...
    def min_notional(self, symbol):
        ...
        logging.info(f"[[{symbol}]] Function min_notional called")
    ...
    def get_effective_leverage(self, user_defined_leverage, symbol, side):
        ...
        logging.info(f"[[{symbol}]] Function get_effective_leverage called")
    ...
    def get_min_qty(self, symbol):
        ...
        logging.info(f"[[{symbol}]] Function get_min_qty called")
    ...
    def calculate_order_amounts_progressive_distribution(
        ...
        # Log the caller name and formatted call stack
        logging.info(f"[[{symbol}]] Calling function calculate_order_amounts_progressive_distribution (called by {caller_name})")
    ...
    def calculate_order_amounts_aggressive_drawdown(self, symbol: str, total_equity: float, 
        ...
        logging.info(f"[[{symbol}]] Function calculate_order_amounts_aggressive_drawdown called")
    ...
    def calculate_total_amount_notional_ls_properdca(self, symbol, total_equity, best_ask_price, best_bid_price, 
        ...
        logging.info(f"[[{symbol}]] Function calculate_total_amount_notional_ls_properdca called")
    ...
    def calculate_order_amounts_notional_properdca(self, symbol: str, total_amount: float, levels: int, 
        ...
        logging.info(f"[[{symbol}]] Function calculate_order_amounts_notional_properdca called")
    ...
    def calculate_max_positions(self, symbol, total_equity, current_price, max_qty_percent_long, max_qty_percent_short):
        ...
        logging.info(f"[[{symbol}]] Function calculate_max_positions called")
    ...
    def get_symbol_max_leverage(self, symbol):
        ...
        logging.info(f"[[{symbol}]] Function get_symbol_max_leverage called")
    ...
    def check_and_manage_positions(
        ...
        total_equity: float, 
        current_price: float, 
        wallet_exposure_limit_long: float,  # e.g., 0.01 for 1%
        wallet_exposure_limit_short: float, # e.g., 0.0025 for 0.25%
        max_qty_percent_long: float,       # e.g., 5 for 5%
        max_qty_percent_short: float       # e.g., 1 for 1%
    ):
        # logging.info(f"[[{symbol}]] Calling function check_and_manage_positions")
            # Automatically capture the caller name
        stack = inspect.stack()
        caller_name = stack[1].function

        # Log the caller name and formatted call stack
        logging.info(f"[[{symbol}]] Calling function check_and_manage_positions (called by {caller_name})")
        
        # Log the full stack trace with proper formatting
        # formatted_stack = "".join(traceback.format_stack())
        # logging.info(f"[[{symbol}]] Calling function check_and_manage_positions. Call stack:\n{formatted_stack}")

        """
        Checks and manages long and short positions for a given symbol based on wallet exposure limits
        and maximum quantity percentages. Clears grids if exposures exceed predefined limits.

        Parameters:
            long_pos_qty (float): Current quantity of long positions.
            short_pos_qty (float): Current quantity of short positions.
            symbol (str): Trading symbol (e.g., 'DRIFTUSDT').
            total_equity (float): Total trading equity in USD.
            current_price (float): Current price of the symbol in USD.
            wallet_exposure_limit_long (float): Wallet exposure limit for long positions (e.g., 0.01 for 1%).
            wallet_exposure_limit_short (float): Wallet exposure limit for short positions (e.g., 0.0025 for 0.25%).
            max_qty_percent_long (float): Maximum quantity percentage allowed for long positions (e.g., 5 for 5%).
            max_qty_percent_short (float): Maximum quantity percentage allowed for short positions (e.g., 1 for 1%).
        """
        try:
            logging.info(f"[[{symbol}]] Checking and managing positions.")

            # Calculate maximum allowed quantities based on wallet exposure limits
            ...
            # Calculate maximum wallet exposure in USD
            max_wallet_exposure_long_dollar = total_equity * wallet_exposure_limit_long
            max_wallet_exposure_short_dollar = total_equity * wallet_exposure_limit_short

            # Calculate current position exposure percentages
            long_exposure_percent = (long_pos_qty * current_price / total_equity) * 100
            short_exposure_percent = (short_pos_qty * current_price / total_equity) * 100

            # Calculate current position exposure in USD
            current_long_exposure_dollar = long_pos_qty * current_price
            current_short_exposure_dollar = short_pos_qty * current_price

            # Log position utilization
            logging.info(f"[[{symbol}]]- Position utilization:")
            logging.info(f"[[{symbol}]]  Long position exposure: {long_exposure_percent:.2f}% of total equity "
                        f"(${current_long_exposure_dollar:.2f} USD)")
            logging.info(f"[[{symbol}]]  Short position exposure: {short_exposure_percent:.2f}% of total equity "
                        f"(${current_short_exposure_dollar:.2f} USD)")

            # Log configuration details
            logging.info(f"[[{symbol}]] Configuration for symbol:")
            ...
            logging.info(f"[[{symbol}]] Maximum allowed positions for symbol:")
            ...
            logging.info(f"[[{symbol}]]  - Max quantity for short: {max_qty_short:.4f} units "
                        f"(${max_wallet_exposure_short_dollar:.2f} USD)")
            logging.info(f"[[{symbol}]]  - Long position quantity: {long_pos_qty} units "
                        f"(${current_long_exposure_dollar:.2f} USD)")
            logging.info(f"[[{symbol}]]  - Short position quantity: {short_pos_qty} units "
                        f"(${current_short_exposure_dollar:.2f} USD)")

            # Manage Long Positions
            if long_exposure_percent > max_qty_percent_long:
                logging.info(
                    f"[[{symbol}]] Long position exposure ({long_exposure_percent:.2f}%) exceeds the maximum allowed "
                    f"({max_qty_percent_long}%). Clearing long grid."
                )
                self.clear_grid(symbol, 'buy')
                self.active_grids.discard(symbol)
                self.max_qty_reached_symbol_long.add(symbol)
            elif symbol in self.max_qty_reached_symbol_long and long_exposure_percent <= max_qty_percent_long:
                logging.info(
                    f"[[{symbol}]] Long position exposure ({long_exposure_percent:.2f}%) is below the maximum allowed "
                    f"({max_qty_percent_long}%). Removing from max_qty_reached_symbol_long."
                )
                self.max_qty_reached_symbol_long.remove(symbol)

            # Manage Short Positions
            if short_exposure_percent > max_qty_percent_short:
                logging.info(
                    f"[[{symbol}]] Short position exposure ({short_exposure_percent:.2f}%) exceeds the maximum allowed "
                    f"({max_qty_percent_short}%). Clearing short grid."
                )
                self.clear_grid(symbol, 'sell')
                self.active_grids.discard(symbol)
                self.max_qty_reached_symbol_short.add(symbol)
            elif symbol in self.max_qty_reached_symbol_short and short_exposure_percent <= max_qty_percent_short:
                logging.info(
                    f"[[{symbol}]] Short position exposure ({short_exposure_percent:.2f}%) is below the maximum allowed "
                    f"({max_qty_percent_short}%). Removing from max_qty_reached_symbol_short."
                )
                self.max_qty_reached_symbol_short.remove(symbol)

        except Exception as e:
            logging.error(f"Exception caught in check_and_manage_positions: {e}")
    ...
    def calculate_total_amount_notional_ls(self, symbol, total_equity, best_ask_price, best_bid_price, 
        ...
        logging.info(f"[[{symbol}]] Function calculate_total_amount_notional_ls called")
    ...
    def calculate_order_amounts_notional(self, symbol: str, total_amount: float, levels: int, strength: float, qty_precision: float, enforce_full_grid: bool) -> List[float]:
        ...
        logging.info(f"[[{symbol}]] Function calculate_order_amounts_notional called")
    ...
    def initiate_spread_entry(self, symbol, open_orders, long_dynamic_amount, short_dynamic_amount, long_pos_qty, short_pos_qty):
        ...
        logging.info(f"[[{symbol}]] Function initiate_spread_entry called")
        order_book = self.exchange.get_orderbook(symbol)
    ...
    def get_order_book_imbalance(self, symbol):
        ...
        logging.info(f"[[{symbol}]] Function get_order_book_imbalance called")
        order_book = self.exchange.get_orderbook(symbol)
    ...
    def identify_walls(self, order_book, type="buy"):
        ...
        logging.info("Function identify_walls called")
    ...
    def print_order_book_imbalance(self, symbol):
        ...
        logging.info(f"[[{symbol}]] Function print_order_book_imbalance called")
    ...
    def log_order_book_walls(self, symbol, interval_in_seconds):
        ...
        logging.info(f"[[{symbol}]] Function log_order_book_walls called")
        """
    ...
    def start_wall_logging(self, symbol):
        ...
        logging.info(f"[[{symbol}]] Function start_wall_logging called")
        """
    ...
    def bybit_turbocharged_entry_maker_walls(self, symbol, trend, mfi, one_minute_volume, five_minute_distance, min_vol, min_dist, take_profit_long, take_profit_short, long_dynamic_amount, short_dynamic_amount, long_pos_qty, short_pos_qty, long_pos_price, short_pos_price):
        ...
        logging.info(f"[[{symbol}]] Function bybit_turbocharged_entry_maker_walls called")
    ...
    def bybit_turbocharged_entry_maker(self, open_orders, symbol, trend, mfi, one_minute_volume: float, five_minute_distance: float, min_vol, min_dist, take_profit_long, take_profit_short, long_dynamic_amount, short_dynamic_amount, long_pos_qty, short_pos_qty, long_pos_price, short_pos_price, should_long, should_add_to_long, should_short, should_add_to_short):
        ...
        logging.info(f"[[{symbol}]] Function bybit_turbocharged_entry_maker called")
    ...
    def bybit_hedge_initial_entry_maker_hma(self, open_orders: list, symbol: str, trend: str, hma_trend: str, mfi: str, one_minute_volume: float, five_minute_distance: float, min_vol: float, min_dist: float, long_dynamic_amount: float, short_dynamic_amount: float, long_pos_qty: float, short_pos_qty: float, should_long: bool, should_short: bool):
        ...
        logging.info(f"[[{symbol}]] Function bybit_hedge_initial_entry_maker_hma called")
    ...
    def bybit_hedge_entry_maker_v2(self, symbol: str, trend: str, mfi: str, one_minute_volume: float, five_minute_distance: float, min_vol: float, min_dist: float, long_dynamic_amount: float, short_dynamic_amount: float, long_pos_qty: float, short_pos_qty: float, long_pos_price: float, short_pos_price: float, should_long: bool, should_short: bool, should_add_to_long: bool, should_add_to_short: bool):
        ...
        logging.info(f"[[{symbol}]] Function bybit_hedge_entry_maker_v2 called")
    ...
    def bybit_hedge_entry_maker_eritrend(self, symbol: str, trend: str, eri: str, one_minute_volume: float, five_minute_distance: float, min_vol: float, min_dist: float, long_dynamic_amount: float, short_dynamic_amount: float, long_pos_qty: float, short_pos_qty: float, long_pos_price: float, short_pos_price: float, should_long: bool, should_short: bool, should_add_to_long: bool, should_add_to_short: bool):
        ...
        logging.info(f"[[{symbol}]] Function bybit_hedge_entry_maker_eritrend called")
