        ...
        try:
            ...
            while self.running_long or self.running_short:
                ...
                try:
                    ...
                    logging.warning(f"[[{symbol}]] Fetched total equity could not be converted to float: {fetched_total_equity}. Resorting to last known equity.")
                    fetched_total_equity = None
                ...
                if current_time - last_equity_fetch_time > equity_refresh_interval or fetched_total_equity == 0.0:
                    if fetched_total_equity is not None and fetched_total_equity > 0.0:
                        ...
                        logging.warning("[[{symbol}]] Failed to fetch valid total_equity or received 0.0. Using last known value.")
                    ...
                    if total_equity is None:
                        logging.error("[[{symbol}]] This should not happen as total_equity should never be None. Skipping this iteration.")
                ...
                logging.info(f"[[{symbol}]] Open symbols: {open_symbols}")
                logging.info(f"[[{symbol}]] Current rotator symbols ({len(rotator_symbols_standardized)}): {list(rotator_symbols_standardized)[:5]}...")
                ...
                logging.info(f"[[{symbol}]] Rotator symbols standardized: ({len(rotator_symbols_standardized)}): {list(rotator_symbols_standardized)[:5]}...")
                ...
                if symbol in rotator_symbols_standardized or (symbol in open_symbols or trading_allowed): # and instead of or
                    ...
                    logging.info(f"[[{symbol}]] Rotator symbols ({len(rotator_symbols_standardized)}): {list(rotator_symbols_standardized)[:5]}...")
                    logging.info(f"[[{symbol}]] Open symbols: {open_symbols}")
