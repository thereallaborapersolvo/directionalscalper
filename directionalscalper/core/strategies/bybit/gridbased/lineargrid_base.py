...
class LinearGridBaseFutures(BybitStrategy):
    def __init__(self, exchange, manager, config, symbols_allowed=None, rotator_symbols_standardized=None, mfirsi_signal=None):
        thread_id = threading.current_thread().ident  # Obtain current thread ID
        self.action = None  # Initialize action as None or a default value
        super().__init__(exchange, config, manager, symbols_allowed, thread_id=thread_id, action=self.action)
        ...
        self.symbol = None  # Will be set in run()
        # self.action = None  # Initialize action as None or a default value
