...
# logger = Logger(logger_name="BybitQuickScalpTrendDynamicTP", filename="BybitQuickScalpTrendDynamicTP.log", stream=True)
logger = Logger(logger_name="BybitQuickScalpTrendDynamicTP", filename="BybitQuickScalpTrendDynamicTP.log", stream=True)
...
class BybitQuickScalpTrendDynamicTP(BybitStrategy):
    ...
    def run(self, symbol, rotator_symbols_standardized=None, mfirsi_signal=None, action=None):
        try:
            ...
            if symbol_locks[standardized_symbol][action].acquire(blocking=False):
                logger.info(f"Lock acquired for symbol {standardized_symbol} action {action} by thread {current_thread_id}")
                try:
                    if action == "long":
                    ...
                    logger.info(f"Lock released for symbol {standardized_symbol} action {action} by thread {current_thread_id}")
            else:
                logger.info(f"Failed to acquire lock for symbol {standardized_symbol} action {action}")
        except Exception as e:
            logger.error(f"Exception in run function: {e}")
    ...
    def run_single_symbol(self, symbol, rotator_symbols_standardized=None, mfirsi_signal=None, action=None):
        try:
            ...
            logger.info(f"Current leverage: {self.current_leverage}")

            logger.info(f"Max leverage for {symbol}: {self.max_leverage}")
            ...
            logger.info(f"Running for symbol (inside run_single_symbol method): {symbol}")
            ...
            while self.running_long or self.running_short:
                ...
                logger.info(f"Trading {symbol} in while loop in obstrategy with long: {self.running_long}")
                logger.info(f"Trading {symbol} in while loop in obstrategy with short: {self.running_short}")

                # Example condition to stop the loop
                if action == "long" and not self.running_long:
                    logger.info(f"Killing thread for {symbol} because not running long")
                    break
                if action == "short" and not self.running_short:
                    logger.info(f"Killing thread for {symbol} because not running short")
                    break
                ...
                logger.info(f"Max USD value: {self.max_usd_value}")
            
                # Log which thread is running this part of the code
                thread_id = threading.get_ident()
                logger.info(f"[Thread ID: {thread_id}] In while true loop {symbol}")
                ...
                for position in open_position_data:
                    ...
                    if 'size' in info and 'side' in info and 'avgPrice' in info and 'liqPrice' in info:
                        ...
                        logger.warning(f"Missing required keys in position info for {position_symbol}")
                ...
                if current_time - last_equity_fetch_time > equity_refresh_interval or total_equity is None:
                    ...
                    logger.info(f"Type of total_equity: {type(total_equity)}")
                ...
                logger.info(f"Funding check on {symbol} : {funding_check}")
                ...
                logger.info(f"Open symbols: {open_symbols}")
                logger.info(f"Current rotator symbols: {rotator_symbols_standardized}")
                symbols_to_manage = [s for s in open_symbols if s not in rotator_symbols_standardized]
                logger.info(f"Symbols to manage {symbols_to_manage}")
                
                #logger.info(f"Open orders for {symbol}: {open_orders}")

                logger.info(f"Symbols allowed: {self.symbols_allowed}")
                ...
                logger.info(f"Initialized symbols: {list(self.initialized_symbols)}")
                ...
                logger.info(f"Rotator symbols standardized: {rotator_symbols_standardized}")

                symbol_precision = self.exchange.get_symbol_precision_bybit(symbol)

                logger.info(f"Symbol precision for {symbol} : {symbol_precision}")
                ...
                logger.info(f"Previous long pos qty for {symbol} : {previous_long_pos_qty}")
                logger.info(f"Previous short pos qty for {symbol} : {previous_short_pos_qty}")

                logger.info(f"Current long pos qty for {symbol} {long_pos_qty}")
                logger.info(f"Current short pos qty for {symbol} {short_pos_qty}")

                # Check if a position has been closed
                if previous_long_pos_qty > 0 and long_pos_qty == 0:
                    logger.info(f"Long position closed for {symbol}. Canceling long grid orders.")
                    self.cancel_grid_orders(symbol, "buy")
                    if short_pos_qty == 0:
                        logger.info(f"No open positions for {symbol}. Removing from shared symbols data.")
                    ...
                    logger.info(f"Short position closed for {symbol}. Canceling short grid orders.")
                ...
                if self.check_position_inactivity(symbol, inactive_pos_time_threshold, long_pos_qty, short_pos_qty, previous_long_pos_qty, previous_short_pos_qty):
                    logger.info(f"No open positions for {symbol} in the last {inactive_pos_time_threshold} seconds. Terminating the thread.")
                    shared_symbols_data.pop(symbol, None)
                    break
                ...
                if not self.running_long and not self.running_short:
                    ...
                    logger.info("Both long and short operations have terminated. Exiting the loop.")
                    break
                
                # Determine if positions have just been closed
                if previous_long_pos_qty > 0 and long_pos_qty == 0:
                    logger.info(f"All long positions for {symbol} were recently closed. Checking for inactivity.")
                    inactive_long = True
                else:
                    inactive_long = False

                if previous_short_pos_qty > 0 and short_pos_qty == 0:
                    logger.info(f"All short positions for {symbol} were recently closed. Checking for inactivity.")
                    inactive_short = True
                else:
                ...
                # if inactive_long:
                #     logger.info(f"No active long positions and previous positions were closed for {symbol}. Terminating long operations.")
                #     self.running_long = False
                #     self.cancel_grid_orders(symbol, "buy")
                ...
                # if inactive_short:
                #     logger.info(f"No active short positions and previous positions were closed for {symbol}. Terminating short operations.")
                #     self.cancel_grid_orders(symbol, "sell")
                ...
                # logger.info(f"Terminate long: {terminate_long}, Terminate short: {terminate_short}")

                # try:
                #     if terminate_long:
                #         logger.info(f"Should terminate long orders for {symbol}")
                #         self.cancel_grid_orders(symbol, "buy")
                #         self.cleanup_before_termination(symbol)
                #         self.running_long = False  # Set the flag to stop the long thread
                #     if terminate_short:
                #         logger.info(f"Should terminate short orders for {symbol}")
                #         self.cancel_grid_orders(symbol, "sell")
                #         self.cleanup_before_termination(symbol)
                #         self.running_short = False  # Set the flag to stop the short thread
                # except Exception as e:
                #     logger.info(f"Exception caught in termination {e}")

                if not self.running_long and not self.running_short:
                    logger.info("Both long and short operations have ended. Preparing to exit loop.")
                ...
                if symbol in rotator_symbols_standardized or (symbol in open_symbols or trading_allowed): # and instead of or
                    ...
                    logger.info(f"{symbol} ERI Trend: {eri_trend}")

                    logger.info(f"{symbol} MFIRSI Signal: {mfirsi_signal}")
                    ...
                    logger.info(f"Long liquidation price for {symbol}: {long_liquidation_price}")
                    logger.info(f"Short liquidation price for {symbol}: {short_liquidation_price}")

                    logger.info(f"Rotator symbol trading: {symbol}")
                                
                    logger.info(f"Rotator symbols: {rotator_symbols_standardized}")
                    logger.info(f"Open symbols: {open_symbols}")

                    logger.info(f"Long pos qty {long_pos_qty} for {symbol}")
                    logger.info(f"Short pos qty {short_pos_qty} for {symbol}")
                    ...
                    logger.info(f"Short dynamic amount: {short_dynamic_amount} for {symbol}")
                    ...
                    try:
                        ...
                        logger.info(f"Failsafe failed: {e}")
                    ...
                    try:
                        ...
                        logger.info(f"Hardened grid AR exception caught {e}")
                    ...
                    logger.info(f"Five minute volume for {symbol} : {five_minute_volume}")
                    ...
                    logger.info(f"Open TP order count {tp_order_counts}")
                    ...
                    if long_pos_qty > 0:
                        try:
                            ...
                            logger.info(f"Long UPNL for {symbol}: {long_upnl}")
                        except Exception as e:
                            logger.info(f"Exception fetching Long UPNL for {symbol}: {e}")
                    ...
                    if short_pos_qty > 0:
                        try:
                            ...
                            logger.info(f"Short UPNL for {symbol}: {short_upnl}")
                        except Exception as e:
                            logger.info(f"Exception fetching Short UPNL for {symbol}: {e}")
                    ...
                    try:
                        ...
                        logger.info(f"Something is up with the variables in for the strategy {e}")
                    

                    logger.info(f"Long tp counts: {long_tp_counts}")
                    logger.info(f"Short tp counts: {short_tp_counts}")

                    logger.info(f"Long pos qty {long_pos_qty} for {symbol}")
                    logger.info(f"Short pos qty {short_pos_qty} for {symbol}")

                    logger.info(f"Long take profit {long_take_profit} for {symbol}")
                    logger.info(f"Short take profit {short_take_profit} for {symbol}")

                    logger.info(f"Long TP order count for {symbol} is {tp_order_counts['long_tp_count']}")
                    logger.info(f"Short TP order count for {symbol} is {tp_order_counts['short_tp_count']}")

                    current_latest_time = datetime.now()
                    logger.info(f"Current time: {current_latest_time}")
                    logger.info(f"Next long TP update time: {self.next_long_tp_update}")
                    logger.info(f"Next short TP update time: {self.next_short_tp_update}")
