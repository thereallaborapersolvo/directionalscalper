...
# logger = Logger(logger_name="BybitMMOneMinuteQFLMFIERIAutoHedgeWallsATR", filename="BybitMMOneMinuteQFLMFIERIAutoHedgeWallsATR.log", stream=True)
logger = Logger(logger_name="BybitMMOneMinuteQFLMFIERIAutoHedgeWallsATR", filename="BybitMMOneMinuteQFLMFIERIAutoHedgeWallsATR.log", stream=True)
        ...
        try:
            ...
            if self.config.dashboard_enabled:
                try:
                    ...
                    logger.error(f"File not found: {dashboard_path}")
            ...
            while True:
                ...
                #logger.info(f"Open position data: {open_position_data}")
                ...
                if symbol in rotator_symbols_standardized or (symbol in open_symbols or trading_allowed): # and instead of or
                    ...
                    # logger.info(f"Long liquidation price for {symbol}: {long_liq_price}")
                    # logger.info(f"Short liquidation price for {symbol}: {short_liq_price}")
                    ...
                    try:
                        ...
                        logger.info(f"Exception caught in autoreduce: {e}")
                    ...
                    logger.info(f"Open TP order count {open_tp_order_count}")
                    ...
                    if long_pos_qty > 0 and long_take_profit is not None and tp_order_counts['long_tp_count'] == 0:
                        logger.info(f"Placing long TP order for {symbol} at {long_take_profit} with {long_pos_qty}")
                    ...
                    if short_pos_qty > 0 and short_take_profit is not None and tp_order_counts['short_tp_count'] == 0:
                        logger.info(f"Placing short TP order for {symbol} at {short_take_profit} with {short_pos_qty}")
            ...
            logger.info(f"Exception caught in onemin_mfi_eri_autohedge strategy {e}")